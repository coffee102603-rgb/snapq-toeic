...
/* ✅ '분홍 띠'가 pseudo-element(::before/::after)로 만들어질 때 차단 */
...
/* ✅ 전역 outline/shadow 제거(안전) */
html *{
  outline: none !important;
  box-shadow: none !important;
}

/* ✅ 혹시 style 속성으로 hotpink 류가 박혀있으면 강제 제거(배경까지) */
*[style*="hotpink"], *[style*="magenta"], *[style*="fuchsia"],
*[style*="#ff00ff"], *[style*="#f0f"], *[style*="rgb(255,0,255)"]{
  outline: none !important;
  box-shadow: none !important;
  border: 0 !important;
  background: transparent !important;
...
/* ✅ P7 글자 크기(클래스 있을 때만) */
:root{ --p7-pass: 25px; --p7-qtit: 28px; --p7-lh: 1.50; }
.p7-card, .p7-card *{ font-size: var(--p7-pass) !important; line-height: var(--p7-lh) !important; }
.p7-q-title, .p7-q-title *,
.p7-question, .p7-question *,
.p7-qtext, .p7-qtext *{ font-size: var(--p7-qtit) !important; line-height: var(--p7-lh) !important; }

/* ===== /P7_FORCE_TOPCSS_V1 V2 ===== */
  /* ===== P7_END_BUTTONS_ANCHOR_CSS_V1 ===== */

  /* NAV 3버튼: 더 납작 + 글자 조금 작게 (앵커 이후에 나오는 stButton 전부) */
  #p7-end-nav-anchor ~ div [data-testid="stButton"] > button{
  ...
  /* 리매치 버튼: 리매치 앵커 이후는 다시 크게 덮어쓰기 */
  #p7-end-rematch-anchor ~ div [data-testid="stButton"] > button{
    ...
    /* ===== P7_END_BUTTONS_ANCHOR_CSS_V2 (DOM-SAFE / STREAMLIT) ===== */
  /* 핵심: stMarkdownContainer(앵커 포함) 기준으로 이후 버튼만 잡는다 (:has 사용) */

  /* DEBUG(필요 시 1분만 켜고 확인 후 주석처리): 앵커가 잡히면 테두리가 보임 */
  /* div[data-testid="stMarkdownContainer"]:has(#p7-end-nav-anchor){ outline: 3px solid rgba(0,200,255,0.9) !important; } */
  /* div[data-testid="stMarkdownContainer"]:has(#p7-end-rematch-anchor){ outline: 3px solid rgba(255,120,0,0.95) !important; } */

  /* NAV 3버튼: 더 납작 + 글자 더 작게 */
  div[data-testid="stMarkdownContainer"]:has(#p7-end-nav-anchor)
  ...
  /* 리매치: 더 크게(🔥 P7 게임 한 판 더!) */
  div[data-testid="stMarkdownContainer"]:has(#p7-end-rematch-anchor)
  ~ div [data-testid="stButton"] > button{
  ...
  /* 혹시 columns/컨테이너가 한 겹 더 들어가도 잡히도록 보강 */
  div[data-testid="stMarkdownContainer"]:has(#p7-end-nav-anchor)
  ~ div *[data-testid="stButton"] > button{
    padding: 6px 8px !important;
    font-size: 14px !important;
  ...
  /* ===== /P7_END_BUTTONS_ANCHOR_CSS_V2 ===== */
/* ===== P7_REWARD_DOM_SAFE_CSS_V2 (VERTICALBLOCK:has ANCHOR / HARD FORCE) ===== */
/* 디버그: 앵커가 잡히면 '보상패널 영역'에 테두리가 생김(성공확인용) */
div[data-testid="stVerticalBlock"]:has(#p7-reward-anchor){
  outline: 3px solid rgba(255,0,180,0.85) !important;
  outline-offset: 4px !important;
}

/* 1) 보상패널 블록 안의 "모든 텍스트" 강제 축소 */
div[data-testid="stVerticalBlock"]:has(#p7-reward-anchor) *{
  font-size: 14px !important;
...
/* 2) 제목만 약간 크게 (가독성 유지) */
div[data-testid="stVerticalBlock"]:has(#p7-reward-anchor) h1{ font-size: 22px !important; }
div[data-testid="stVerticalBlock"]:has(#p7-reward-anchor) h2{ font-size: 20px !important; }
div[data-testid="stVerticalBlock"]:has(#p7-reward-anchor) h3{ font-size: 18px !important; }

/* 3) 탭 라벨(해석/정답/단어) */
div[data-testid="stVerticalBlock"]:has(#p7-reward-anchor) button[role="tab"] *{
  font-size: 14px !important;
  font-weight: 900 !important;
}

/* 4) 경고/알림 박스(stAlert) 안도 강제 축소 */
div[data-testid="stVerticalBlock"]:has(#p7-reward-anchor) div[data-testid="stAlert"] *{
  font-size: 14px !important;
  line-height: 1.25 !important;
}
/* ===== /P7_REWARD_DOM_SAFE_CSS_V2 ===== */
/* ===== /P7_END_BUTTONS_ANCHOR_CSS_V1 ===== */
</style>
<div class="p7-css-badge">P7 CSS ACTIVE ✅</div>
...
if st.session_state.p7_done:
    # ==============================
    # P7 END PANEL (옵션 A: UI ONLY)
    # - 8문장(=Step3) 완료 후: 보상/다음 전장
    # - 시간종료/실패: 리트라이 중심
    # ==============================
    ...
    if status == "CLEAR" and step >= 3:

        st.markdown("""
        <style>
        /* === P7 END PANEL: HUD OFF + CONTRAST UP (SAFE) === */
        .p7-hudbar, .p7-hud-gauge, .p7-hud-left, .p7-hud-right, .p7-hud, .p7-timer-wrap { display:none !important; }
        .p7-end-card, .p7-end-card *{ color: rgba(15,23,42,0.92) !important; }
        ...
        .p7-end-nav div[data-testid="stButton"]>button:hover{ border-color: rgba(0,229,255,0.55) !important; }
        div[data-testid="stAlert"] *{ color: rgba(15,23,42,0.90) !important; }
        </style>
        """, unsafe_allow_html=True)
        st.success("🏁 MISSION CLEAR — 8문장 누적 완료!")
        acc = max(0, 3 - miss) / 3.0
        if acc >= 1.0 and rem >= 45:
            rank, xp, coin = "S", 120, 60
        elif acc >= 1.0:
            rank, xp, coin = "A", 100, 50
        elif acc >= 0.67:
            rank, xp, coin = "B", 80, 40
        else:
            rank, xp, coin = "C", 60, 30

        st.markdown(f"""
        <div class="p7-card p7-end-card" style="border:1px solid rgba(0,229,255,0.32); background: linear-gradient(180deg, rgba(0,229,255,0.10), rgba(27,124,255,0.08));">
          <div style="font-weight:950; font-size:1.25em;">🎖 전투 등급: {rank}</div>
          <div style="margin-top:6px; opacity:0.95;">MISS: <b>{miss}</b> · COMBO: <b>{combo}</b> · 남은 시간: <b>{rem}s</b></div>
          <div style="margin-top:10px; display:flex; gap:10px; flex-wrap:wrap;">
            <span style="padding:8px 12px; border-radius:999px; border:1px solid rgba(148,163,184,0.22); background:rgba(255,255,255,0.06);">⚡ XP +{xp}</span>
            <span style="padding:8px 12px; border-radius:999px; border:1px solid rgba(148,163,184,0.22); background:rgba(255,255,255,0.06);">🪙 COIN +{coin}</span>
            <span style="padding:8px 12px; border-radius:999px; border:1px solid rgba(148,163,184,0.22); background:rgba(255,255,255,0.06);">🎁 보상: 해석/정답/단어 패널 OPEN</span>
          </div>
        </div>
        """, unsafe_allow_html=True)
                ...
                # =============================
        # P7_REWARD_PANEL_V1 (CLICK -> OPEN / SAVE ARMORY)
        # =============================
        if "p7_reward_open" not in st.session_state:
            st.session_state.p7_reward_open = False
        if "p7_reward_saved" not in st.session_state:
            st.session_state.p7_reward_saved = False

        col_r1, col_r2 = st.columns([1,1], gap="small")
        with col_r1:
            if st.button("🎁 보상 열기 / 닫기", use_container_width=True, key="p7_reward_toggle"):
                st.session_state.p7_reward_open = not st.session_state.p7_reward_open
        with col_r2:
            # 저장고 파일 경로(프로젝트 루트 기준)
            arm_dir = os.path.join("data","armory")
            arm_file = os.path.join(arm_dir, "p7_vocab.jsonl")
            st.caption(f"📦 저장고: {arm_file}")

        if st.session_state.p7_reward_open:
            st.markdown("#### 🎁 보상 패널 (해석 / 정답 / 단어)")
            st.markdown('<div id="p7-reward-anchor"></div>', unsafe_allow_html=True)
            tab1, tab2, tab3 = st.tabs(["🈯 해석", "✅ 정답", "🧠 단어(저장고)"])

            # --- (A) 해석: 현재 샘플 세트 기준 (8문장) ---
            #     ⚠️ 여기 KO는 샘플로 제공. 나중에 세트별로 교체/확장하면 됨.
            ...
            with tab1:
                need = 2 if st.session_state.p7_step == 1 else (5 if st.session_state.p7_step == 2 else 8)
                st.info(f"현재 누적 문장: {need}문장")
                for i in range(need):
            ...
            def _load_existing_words(path: str) -> set:
            ...
            def _append_jsonl(path: str, rows: list[dict]) -> int:
            ...
            with tab3:
                st.warning("🧠 아래 단어들은 ‘저장고(Secret Armory)’에 저장할 수 있습니다.")
                for v in vocab_entries:
                    st.markdown(f"**{v['word']}** — {v['meaning']}")
                    st.caption(f"CORE: {v['core']}  |  EX: {v['example']}")
                    st.markdown("---")

                col_s1, col_s2 = st.columns([1,1], gap="small")
                with col_s1:
                    if st.button("📥 단어 저장고에 저장", use_container_width=True, key="p7_vocab_save_btn"):
                        existing = _load_existing_words(arm_file)
                        ...
                        for v in vocab_entries:
                            ...
                            rows.append({
                                "ts": now,
                                "source": "P7_END_REWARD",
                                "word": v["word"],
                                "meaning": v["meaning"],
                                "core": v.get("core",""),
                                "example": v.get("example",""),
                            })
                        n = _append_jsonl(arm_file, rows) if rows else 0
                        st.session_state.p7_reward_saved = True
                        st.success(f"✅ 저장 완료: {n}개 (중복 제외)")
                with col_s2:
                    if st.button("🧽 이번 판 저장 표시 초기화", use_container_width=True, key="p7_vocab_reset_saved"):
                        st.session_state.p7_reward_saved = False
                        st.info("표시만 초기화했습니다(파일 삭제는 아님).")

        # =============================
        # /P7_REWARD_PANEL_V1
        # =============================
        st.markdown('<div class="p7-done-cta-anchor"></div>', unsafe_allow_html=True)
        st.markdown("### 🔜 다음 전장 이동")
        st.markdown('<div id="p7-end-nav-anchor"></div>', unsafe_allow_html=True)
        st.markdown('<div class="p7-end-nav">', unsafe_allow_html=True)
        c1, c2, c3 = st.columns(3, gap="small")
        with c1:
            if st.button("🏠 MAIN HUB", use_container_width=True, key="p7_done_to_hub"):
                try:
                    st.switch_page("main_hub.py")
                except Exception:
                    st.warning("main_hub.py 경로를 찾지 못했습니다.")
        with c2:
            if st.button("📊 SCOREBOARD", use_container_width=True, key="p7_done_to_score"):
                try:
                    st.switch_page("pages/04_Scoreboard.py")
                except Exception:
                    st.warning("pages/04_Scoreboard.py 경로를 찾지 못했습니다.")
        with c3:
            if st.button("🧰 비밀 무기고", use_container_width=True, key="p7_done_to_armory"):
                try:
                    st.switch_page("pages/03_Secret_Armory_Main.py")
                except Exception:
                    st.warning("pages/03_Secret_Armory_Main.py 경로를 찾지 못했습니다.")
        st.markdown("---")
        st.markdown('<div id="p7-end-rematch-anchor"></div>', unsafe_allow_html=True)
        if st.button("🔥 P7 게임 한 판 더!", use_container_width=True, key="p7_done_rematch"):
            reset_battle()
            st.rerun()
...
# --- P7_SAFE_CSS_INJECT_V1 START ---
import streamlit as st
st.markdown("""
<style>
/* ===== P7_SAFE_CSS_INJECT_V1 ===== */

/* (A) 분홍/핫핑크 디버그 테두리/아웃라인/그림자 전역 무력화 */
html, body, body *{
  outline: none !important;
...
/* 혹시 border 자체를 hotpink로 박아놨으면 강제 제거 */
*[style*="hotpink"], *[style*="magenta"], *[style*="fuchsia"],
*[style*="#ff00ff"], *[style*="#f0f"], *[style*="rgb(255,0,255)"]{
  outline: none !important;
  box-shadow: none !important;
