...
if "P7_FORCE_SKIN_V2" in src:
    print("[SKIP] P7_FORCE_SKIN_V2 already present")
    raise SystemExit(0)
...
# We will inject a FORCE skin st.markdown(<style>..</style>) as part of the ALWAYS section.
# Insert right after the first st.markdown("""<style> ... </style>""", unsafe_allow_html=True) call inside inject_css.
...
"""
body2 = body[:insert_at] + FORCE + body[insert_at:]
patched = src[:m.start()] + body2 + src[end:]
open(path, "w", encoding="utf-8").write(patched)
print("[OK] P7_FORCE_SKIN_V2 injected into inject_css() (ALWAYS)")
