...
def init_state():
    ...
    st.session_state.setdefault("p7_history", [])
    st.session_state.setdefault("p7_reward_open", False)
...
.p7-body { padding-top: 0px !important; }
...
.p7-end-nav div[data-testid="stButton"]>button{ min-height: 44px !important; padding: 10px 12px !important; font-size: 15px !important; }
        ...
        if st.session_state.get("p7_reward_open", False):
            tab1, tab2, tab3 = st.tabs(["🈁 해석(영문 확인)", "✅ 정답 리포트", "🗂 단어(자동 추출)"])
            _hist = list(st.session_state.get("p7_history", []))

            with tab1:
                st.caption("※ 이 데모 페이지는 한글 해석 데이터가 없어서, 대신 '영문 지문'을 그대로 보상으로 공개합니다.")
                for i, r in enumerate(_hist, start=1):
                    st.markdown(f"**[STEP {r.get('step', i)}] Passage**")
                    st.write(r.get("passage",""))

            with tab2:
                st.caption("내 선택 vs 정답 — Step별 리포트")
                if not _hist:
                    st.info("아직 기록이 없습니다.")
                for i, r in enumerate(_hist, start=1):
                    opts = r.get("options", [])
                    picked = int(r.get("picked", -1))
                    ans = int(r.get("answer", -1))
                    ok = bool(r.get("correct", False))
                    st.markdown(f"**STEP {r.get('step', i)} · {'✅ CLEAR' if ok else '❌ MISS'}**")
                    st.write("Q:", r.get("question",""))
                    if 0 <= picked < len(opts):
                        st.write("내 선택:", f"({chr(65+picked)}) {opts[picked]}")
                    if 0 <= ans < len(opts):
                        st.write("정답:", f"({chr(65+ans)}) {opts[ans]}")
                    st.divider()

            with tab3:
                st.caption("간단 토큰화(자동 추출) — 실제 SnapQ 단어장과 연결 예정")
                import re as _re
                words=[]
                for r in _hist:
                    txt = (r.get("passage","") + " " + r.get("question",""))
                    words += _re.findall(r"[A-Za-z]{4,}", txt)
                words = sorted(set(w.lower() for w in words))
                if words:
                    st.write(" / ".join(words[:120]))
                    if len(words) > 120:
                        st.caption(f"... +{len(words)-120}개 더")
                else:
                    st.info("추출할 단어가 없습니다.")
...
if choice is not None:
    ...
    # 기록(보상 패널용) — 진행 로직에는 영향 없음 (옵션 A)
    try:
        _hist = st.session_state.get("p7_history", [])
        _hist.append({
            "step": int(st.session_state.get("p7_step", 1)),
            "question": step.question,
            "options": step.options,
            "picked": int(picked),
            "answer": int(step.answer_idx),
            "correct": bool(picked == step.answer_idx),
            "passage": step.passage,
        })
        st.session_state["p7_history"] = _hist
    except Exception:
        pass
