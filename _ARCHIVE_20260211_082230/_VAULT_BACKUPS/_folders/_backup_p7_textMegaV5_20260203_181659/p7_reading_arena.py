...
def inject_css():
  ...
  .p7-zone .p7-zone-body{
    font-size: 22px !important;   /* 18px -> +20% 이상 */
  ...
  @media (max-width: 520px){
    .p7-zone .p7-zone-body{
      font-size: 20px !important; /* 모바일도 크게 */
...
.p7-zone .p7-zone-body,
.p7-zone .p7-zone-body *{
  font-size: 22px !important;   /* 기존 18px 기준 +20% */
...
def render_p7_step(step: int):
    ...
    # --- TEXT MEGA FORCE (V4, mobile readable) ---
    st.markdown(r"""
    <style>
      /* P7_TEXT_MEGA_FORCE_V4_FINAL */

      /* 1) P7 내부 텍스트(지문/문제) : 클래스가 안 먹어도 Streamlit markdown 자체를 잡는다 */
      .p7-zone .p7-zone-body,
      .p7-zone .p7-zone-body *{
      ...
      /* 2) Streamlit markdown 컨테이너(진짜 DOM) 직접 타격 */
      .p7-zone div[data-testid="stMarkdownContainer"] p,
      .p7-zone div[data-testid="stMarkdownContainer"] span,
      .p7-zone div[data-testid="stMarkdownContainer"] li{
        font-size: 26px !important;
        font-weight: 900 !important;
        line-height: 1.65 !important;
      }

      /* 3) 문제 타이틀/질문 줄까지 커지도록(혹시 h 태그로 렌더되는 경우) */
      .p7-zone div[data-testid="stMarkdownContainer"] h1,
      .p7-zone div[data-testid="stMarkdownContainer"] h2,
      .p7-zone div[data-testid="stMarkdownContainer"] h3{
        font-size: 28px !important;
        font-weight: 900 !important;
        line-height: 1.25 !important;
      }

      /* 4) 선택지(라디오) : label 내부 모든 글자 강제 확대 */
      .p7-zone div[role="radiogroup"] label,
      .p7-zone div[role="radiogroup"] label *{
        font-size: 1.35em !important;   /* 선택지 큼 */
        font-weight: 900 !important;
        line-height: 1.35 !important;
      }

      /* 5) 모바일(<=520px)에서 한 단계 더 키움 */
      @media (max-width: 520px){
        .p7-zone .p7-zone-body,
        .p7-zone .p7-zone-body *{
          font-size: 28px !important;
...
st.markdown(
        ...
        <div style='padding:10px 12px;margin:10px 0;border:3px solid #ff2d2d;border-radius:14px;background:rgba(255,45,45,.12);font-weight:900;font-size:18px;'>✅ P7 PASSAGE PROBE HIT: P7_BUILD_20260205_145812</div>\n<div class='p7-zone intel p7-step' style='--p7-step-color:{step_color}; --p7-step-glow:{step_glow};'>
          <div class='p7-zone-body p7-textboost'>{(" ".join(data.sentences)).replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        <div class='p7-zone mission p7-step' style='--p7-step-color:{step_color}; --p7-step-glow:{step_glow};'>
          <div class='p7-zone-body p7-textboost'>{(data.question_en).replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
@media (max-width: 640px){
  .p7-zone .p7-zone-body{ font-size: 22px !important; line-height: 1.6 !important; }
