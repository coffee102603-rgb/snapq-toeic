    # === P7_CUM_SENTENCES_LOCK (2→5→8 ONLY) ===
    all_s = list(getattr(current_set, "all_sentences_en", []) or [])

    # 최후 안전장치: all_sentences_en이 비어있으면 기존 step 데이터를 "증분"으로 정규화
    if not all_s:
        ...
        # step2/step3가 "전체(누적)"로 들어있는 경우가 많으니, 뒤쪽 3문장만 증분으로 사용
        add2 = s2[-3:] if len(s2) >= 3 else s2
        add3 = s3[-3:] if len(s3) >= 3 else s3

        all_s = s1 + add2 + add3

    if step == 1:
        cum_sentences = all_s[:2]
    elif step == 2:
        cum_sentences = all_s[:5]
    else:
        cum_sentences = all_s[:8]

    cum_text = " ".join([str(x) for x in cum_sentences])
    cum_text = cum_text.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")
    # === /P7_CUM_SENTENCES_LOCK ===# === P7_CUM_SENTENCES_SAFE_V3 applied at 20260203_194717 ===
