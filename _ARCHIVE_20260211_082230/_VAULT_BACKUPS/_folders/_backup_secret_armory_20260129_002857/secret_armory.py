...
#
# IMPORTANT COMPAT:
# pages/03_Secret_Armory_Main.py imports: from app.arenas.secret_armory import render
# So we provide render() which calls app().
# ============================================================
...
# .../snapq_toeic/app/arenas/secret_armory.py
...
.snapq-title { font-weight: 900; letter-spacing: -0.6px; font-size: 30px; line-height: 1.1; margin: 8px 0 2px 0; }
...
.train-word { font-size: 18px; font-weight: 900; }
.train-meaning { font-size: 14px; font-weight: 800; opacity: 0.95; }
...
def render_train(words_all: List[ArmoryWord]) -> None:
    ...
    with c1:
        st.session_state["armory_query"] = st.text_input(
            "🔎 검색 (단어/뜻/예문/메모/태그)",
            value=st.session_state.get("armory_query", ""),
            key="armory_query_train",
        )
    with c2:
        cur_tag = st.session_state.get("armory_tag", "ALL")
        idx = tags.index(cur_tag) if cur_tag in tags else 0
        st.session_state["armory_tag"] = st.selectbox("🏷️ 태그", options=tags, index=idx, key="armory_tag_train")
    with c3:
        sorts = ["RECENT", "A→Z", "MOST_SEEN", "MOST_WRONG"]
        cur_sort = st.session_state.get("armory_sort", "RECENT")
        st.session_state["armory_sort"] = st.selectbox("↕️ 정렬", options=sorts, index=sorts.index(cur_sort), key="armory_sort_train")
...
def render_battle(words_all: List[ArmoryWord]) -> None:
    ...
    with right:
        ...
        st.markdown(
            f'<div class="label">오늘 본 무기</div><div class="kpi">{st.session_state.get("armory_today_seen", 0)}</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            f'<div class="mini">✅ {st.session_state.get("armory_today_correct", 0)} / ❌ {st.session_state.get("armory_today_wrong", 0)}</div>',
            unsafe_allow_html=True,
        )
    ...
    with left:
        ...
        with top[0]:
            st.session_state["armory_query"] = st.text_input(
                "🔎 BATTLE 검색 (단어/뜻/예문/메모/태그)",
                value=st.session_state.get("armory_query", ""),
                key="armory_query_battle",
            )
        with top[1]:
            cur_tag = st.session_state.get("armory_tag", "ALL")
            idx = tags.index(cur_tag) if cur_tag in tags else 0
            st.session_state["armory_tag"] = st.selectbox("🏷️ 태그", options=tags, index=idx, key="armory_tag_battle")
        with top[2]:
            sorts = ["RECENT", "A→Z", "MOST_SEEN", "MOST_WRONG"]
            cur_sort = st.session_state.get("armory_sort", "RECENT")
            st.session_state["armory_sort"] = st.selectbox("↕️ 정렬", options=sorts, index=sorts.index(cur_sort), key="armory_sort_battle")
...
# pages/03_Secret_Armory_Main.py expects render()
# -----------------------------
def render() -> None:
    app()


# Optional alias (extra-safe)
render_page = render
