...
# =========================
ARMORY_BUILD = "ARMORY_SAFE_V2__2_BATTLES__NO_FORCED_RERUN__5WIN_REWARD__20260128"
...
.card .sub{ color:#1f2937 !important; font-weight:900 !important; }
.card .big{ font-size:22px; font-weight:950 !important; color:#0b1220 !important; }
...
def _ensure():
    ...
    ss.setdefault("voca_chunk_idx", 0)
    ss.setdefault("voca_show_meaning", False)
...
def _train(items: List[Dict[str, Any]]):
    ...
    chs = _chunks(it) or ([word] if word else ["(no chunk)"])
    ss["voca_chunk_idx"] = max(0, min(int(ss["voca_chunk_idx"]), len(chs)-1))
    chunk = chs[ss["voca_chunk_idx"]]
...
""", unsafe_allow_html=True)
        ...
        with b1:
            if st.button("📘 뜻 보기", use_container_width=True, key="train_show"):
                ss["voca_show_meaning"] = not bool(ss["voca_show_meaning"])
        with b2:
            if st.button("➡️ 다음", use_container_width=True, key="train_next"):
                ss["voca_show_meaning"] = False
                ss["voca_chunk_idx"] = 0
        ...
        if len(chs) > 1 and st.button("🔁 다른 조각(Chunk) 보기", use_container_width=True, key="train_chunk_next"):
            ss["voca_chunk_idx"] = (ss["voca_chunk_idx"] + 1) % len(chs)

        if ss.get("voca_show_meaning"):
            st.markdown(f"""
...
<div class="card">
  ...
  <div style="margin-top:8px;color:#111827;font-weight:950;">{msg}</div>
</div>
""", unsafe_allow_html=True)
    ...
    if kind == "MEANING_4CHOICE" and not meaning:
        kind = "CHUNK_BLANK"

    if kind == "MEANING_4CHOICE":
        all_meanings = [str(x.get("meaning","") or x.get("ko","") or "").strip() for x in items]
        all_meanings = [m for m in all_meanings if m]
        distract = [m for m in all_meanings if m != meaning]
        rnd2 = random.Random(int(ss["battle_seed"]) + qi + 777)
        rnd2.shuffle(distract)
        opts = [meaning] + distract[:3]
        rnd2.shuffle(opts)
        prompt = f"WORD · {word}"
        answer = meaning
        ...
        chs = _chunks(it) or [word]
        prompt = _blank(chs[0], word)
...
<div class="card">
  ...
  <div style="height:10px;"></div>
  <div class="big"><span class="chip">{chip}</span> · {prompt}</div>
  <div style="margin-top:8px;color:#6b7280;font-weight:800;">정답을 골라라</div>
</div>
""", unsafe_allow_html=True)
