...
/* =========================================================
   HUD v4 FINAL (ONE BAR / 5 ITEMS / GAUGE TIGHT)
   - all 5 items inside top bar including HUB (pill)
   - gauge sits right under bar to save space
   ========================================================= */
.p7-hudbar{
  margin: 0 0 4px 0 !important;
  padding: 6px 10px !important;
...
def render_top_hud():
    ...
    step = i# TOP HUD v4 FINAL (ONE BAR / 5 ITEMS INLINE + GAUGE TIGHT)
    # - STEP / TIME / MISS / COMBO / MAIN HUB are all inside the single top bar
    # - gauge is directly under the bar to save space (English passage is long)

    # HUB link: safest navigation is to the app root (main_hub is the main script)
    hub_href = "/"

    hud_html = textwrap.dedent(
        f"""
        <div class="p7-hudbar">
          <div class="p7-hud-left">
            <span class="p7-chip">🔥 STEP <b>{step}</b>/3</span>
            <span class="{time_cls} p7-time-chip">{ring_svg}<span>⏱ TIME <b>{get_time_display(remaining)}</b></span></span>
            <span class="{miss_cls}">❌ MISS <b>{wrong}</b>/3</span>
            <span class="p7-chip">💣 COMBO <b>{combo.current_combo}</b></span>
          </div>
          <div class="p7-hud-right">
            <a class="p7-hub-link" href="{hub_href}" target="_self">🏠 HUB</a>
          </div>
        </div>
        """
    )
    ...
    # Gauge right under bar (color by urgency)
