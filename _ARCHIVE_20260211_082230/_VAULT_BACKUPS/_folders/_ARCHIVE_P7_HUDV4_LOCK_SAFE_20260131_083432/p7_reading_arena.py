...
/* hide default sidebar space if any */
        ...
        /* =========================
           P7 Mobile Readability + HUD Compact (v1)
           - UI only (CSS/spacing)
           ========================= */

        /* overall padding: push main to top-left */
        .block-container{ padding-top: 0.25rem !important; padding-bottom: 1.0rem !important; padding-left: 1.1rem !important; padding-right: 1.1rem !important; }
        /* thinner divider */
        .p7-divider{ height:1px; background: rgba(255,255,255,0.18); margin: 0.35rem 0 0.55rem 0; }

        /* passage/question cards: stronger contrast */
        .p7-zone{
        ...
        .p7-zone .p7-zone-body{
          ...
          text-shadow: 0 10px 22px rgba(0,0,0,0.32) !important;
        }
        ...
        /* option cards: bigger tap target + higher contrast */
        div[role="radiogroup"] > label{
          background: rgba(255,255,255,0.10) !important;
          border: 1px solid rgba(255,255,255,0.18) !important;
          padding: 16px 16px !important;   /* 터치 영역 확대 */
        ...
        /* HUD wrapper: compact everything on the right */
        .p7-hud-wrap{ transform: scale(0.92); transform-origin: top right; }
        .p7_step_panel{ padding: 10px 10px 8px 10px !important; border-radius: 16px !important; margin-bottom: 6px !important; }
        .p7_step_title{ margin-bottom: 6px !important; font-size: 12px !important; }

        /* make right-side buttons smaller but still full width */
        .p7-hud-wrap button{
          padding: 8px 10px !important;
          border-radius: 12px !important;
        }
        /* reduce vertical gaps between elements */
        .p7-hud-wrap .stButton{ margin-bottom: 0.35rem !important; }



/* =========================
   P7 MOBILE BREATHING PATCH + HUD STICKY
   - 여백/간격/가독성 개선
   - HUD sticky (모바일에서 LOCK IN 항상 보이게)
   ========================= */
.block-container{
...
/* ✅ HUD sticky: keep it visible while scrolling the battlefield */
.p7-hud-sticky{
  position: sticky;
  top: 0.6rem;
  z-index: 50;
...
/* =========================
   P7 OPTION BUTTON CARDS (Tap = Submit)
   ========================= */
.p7-opt-wrap{ margin-top: 10px; }
.p7-opt-wrap div[data-testid="stButton"]{ margin-bottom: 10px; }
.p7-opt-wrap div[data-testid="stButton"] > button{
  width: 100% !important;
  text-align: left !important;
  padding: 16px 16px !important;           /* ~0.3cm */
  border-radius: 18px !important;
...
/* =========================
   A-STRUCT: TOP HUD (sticky) — RIGHT HUD OFF
   ========================= */
.p7-top-hud{
  min-height: 64px;
  position: sticky;
...
.p7-top-left, .p7-top-mid, .p7-top-right{
  display:flex;
  align-items:center;
  gap:10px;
  flex-wrap:wrap;
...
.p7-chip b{ font-size: 14px; }
.p7-top-actions button{
  padding: 8px 10px !important;
  border-radius: 14px !important;
  font-weight: 900 !important;
}
.p7-top-gauge{ margin-top: 8px; }


/* ===============================
   🔥 FINAL VISIBILITY PATCH (A안 유지 + 가독성 100%)
   - existing selectors override only (no markup changes)
   =============================== */

/* Passage (intel) card -> dark glass for contrast */
.p7-zone.intel{
  background: linear-gradient(135deg, rgba(20, 24, 40, 0.92), rgba(30, 40, 70, 0.88)) !important;
...
.p7-zone.intel .p7-zone-body{
  ...
  text-shadow: 0 2px 6px rgba(0,0,0,0.60) !important;
  font-weight: 700 !important;
  font-size: clamp(22px, 3.2vw, 34px) !important;
  line-height: 1.6 !important;
}

/* Question (mission) card -> slightly different dark */
.p7-zone.mission{
...
/* Choices: default = white card + black text (max readability) */
.p7-opt-wrap div[data-testid="stButton"] > button{
...
/* Hover/touch = neon gradient + white text (arcade feel) */
.p7-opt-wrap div[data-testid="stButton"] > button:hover{
...
/* Global markdown container: keep bright */
[data-testid="stMarkdownContainer"]{
  color:#ffffff !important;
}

/* Keep TOP HUD chips readable */
.p7-chip, .p7-chip *{
  color:#ffffff !important;
  opacity: 1 !important;
}
...
/* ===============================
   🚨 FORCE APPLY (Streamlit buttons are NOT inside our <div> wrappers)
   - make ALL Streamlit buttons readable
   =============================== */

/* Every Streamlit st.button */
...
/* Make our TOP HUD icon buttons compact again (they are st.button too)
   We can only target by container: first button row right under .p7-top-hud is inside columns.
   Use a conservative rule: make buttons in the first action row smaller by limiting height.
*/
.p7-top-hud ~ div div[data-testid="stButton"] > button{
...
/* =========================
   P7 TOP HUD COMPACT + TIME PRESSURE (v1)
   ========================= */
.block-container{ padding-top:0.12rem!important; padding-left:0.75rem!important; padding-right:0.75rem!important; }
.p7-top-hud{ margin:0 auto 0px auto!important; padding:6px 8px!important; border-radius:16px!important; min-height:54px!important; top:0.20rem!important; }
.p7-chip{ padding:5px 9px!important; font-size:12.5px!important; }
.p7-chip b{ font-size:13.5px!important; }
.p7-battlefield{ margin-top: 0px!important; }
.p7-mainhub-compact div[data-testid="stButton"] > button{ padding:10px 10px!important; border-radius:14px!important; font-weight:900!important; line-height:1.0!important; }
...
@keyframes p7Shake10{ 0%{transform:translateX(0)} 20%{transform:translateX(-1px)} 40%{transform:translateX(1px)} 60%{transform:translateX(-1px)} 80%{transform:translateX(1px)} 100%{transform:translateX(0)} }
.p7-chip.p7-time-danger{ background:rgba(255,45,45,0.22)!important; border-color:rgba(255,45,45,0.45)!important; box-shadow:0 0 14px rgba(255,45,45,0.18)!important; }
.p7-time-danger .p7-time-svg{ filter:drop-shadow(0 0 7px rgba(255,45,45,.55)); animation:p7Pulse60 .9s ease-in-out infinite; }
.p7-chip.p7-time-critical{ background:rgba(255,45,45,0.30)!important; border-color:rgba(255,45,45,0.55)!important; box-shadow:0 0 18px rgba(255,45,45,0.25)!important; }
.p7-time-critical .p7-time-svg{ filter:drop-shadow(0 0 9px rgba(255,45,45,.65)); animation:p7Pulse30 .75s ease-in-out infinite; }
.p7-chip.p7-time-final{ animation:p7Shake10 .35s linear infinite; }
/* =========================================================
...
/* =========================================================
   ✅ TIME OVERRIDE (FINAL, CLEAN)
   - 반드시 <style> 맨 끝에 둔다.
   ========================================================= */
@keyframes p7TimeAlive {
  0%   { transform: scale(1);   filter: brightness(1.05); }
  50%  { transform: scale(1.06); filter: brightness(1.40); }
  100% { transform: scale(1);   filter: brightness(1.05); }
}
@keyframes p7TimeJitter {
  0%{transform:translateX(0)}
  25%{transform:translateX(-1px)}
  50%{transform:translateX(1px)}
  75%{transform:translateX(-1px)}
...
/* 평상시에도 살아있게 */
.p7-hudbar .p7-time-chip{
  animation: p7TimeAlive .80s ease-in-out infinite !important;
  box-shadow: 0 0 0 1px rgba(255,255,255,.22) inset,
              0 0 30px rgba(120,90,255,.35) !important;
...
.p7-hudbar .p7-time-chip.p7-time-warn{
  ...
  box-shadow: 0 0 0 1px rgba(255,204,0,.22) inset,
              0 0 34px rgba(255,204,0,.24) !important;
}
/* 30초 이하 */
.p7-hudbar .p7-time-chip.p7-time-danger2{
  animation-duration: .52s !important;
  box-shadow: 0 0 0 1px rgba(255,60,60,.26) inset,
              0 0 44px rgba(255,60,60,.35) !important;
}
/* 10초 이하: 떨림 + 더 강함 */
.p7-hudbar .p7-time-chip.p7-time-final2{
  animation: p7TimeJitter .20s linear infinite !important;
  box-shadow: 0 0 0 1px rgba(255,40,40,.30) inset,
              0 0 60px rgba(255,40,40,.50) !important;
}


/* =========================================================
   ✅ P7 TIME PRESSURE FINAL CLEAN (ALWAYS WORKS)
   - broken CSS above? 상관없이 "마지막 승자"로 TIME만 살린다.
   ========================================================= */

@keyframes p7_time_alive {
...
@keyframes p7_time_warn {
  0%{transform:scale(1)} 40%{transform:scale(1.07)} 100%{transform:scale(1)}
}
@keyframes p7_time_danger {
  0%{transform:scale(1)} 20%{transform:scale(1.08)} 40%{transform:scale(.99)} 60%{transform:scale(1.10)} 100%{transform:scale(1)}
}
@keyframes p7_time_final_shake {
...
@keyframes p7_ring_glow {
  0%{filter:drop-shadow(0 0 6px rgba(255,60,60,.30))}
  50%{filter:drop-shadow(0 0 12px rgba(255,60,60,.65))}
  100%{filter:drop-shadow(0 0 6px rgba(255,60,60,.30))}
}

/* TIME 기본: 항상 살아있게 */
.p7-hudbar .p7-time-chip{
  animation: p7_time_alive .88s ease-in-out infinite !important;
  background: rgba(0,0,0,0.30) !important;
  border: 1px solid rgba(255,255,255,0.28) !important;
  box-shadow: 0 0 0 1px rgba(255,255,255,0.12) inset, 0 0 22px rgba(120,90,255,0.28) !important;
}

/* SVG 링은 무조건 보이게 */
.p7-hudbar .p7-time-svg{
  display:inline-block !important;
  width:18px !important; height:18px !important;
}
.p7-hudbar .p7-time-svg circle{ opacity:1 !important; }

/* 60초 ↓ */
.p7-hudbar .p7-time-chip.p7-time-warn{
  animation: p7_time_warn .72s ease-in-out infinite !important;
  box-shadow: 0 0 0 1px rgba(255,204,0,.20) inset, 0 0 26px rgba(255,204,0,.20) !important;
}

/* 30초 ↓ */
.p7-hudbar .p7-time-chip.p7-time-danger2{
  animation: p7_time_danger .55s ease-in-out infinite !important;
  box-shadow: 0 0 0 1px rgba(255,60,60,.24) inset, 0 0 36px rgba(255,60,60,.32) !important;
}

/* 10초 ↓ (TIME 칩만 흔들림) */
.p7-hudbar .p7-time-chip.p7-time-final2{
  animation: p7_time_final_shake .22s linear infinite !important;
  box-shadow: 0 0 0 1px rgba(255,40,40,.30) inset, 0 0 44px rgba(255,40,40,.40) !important;
}

/* 링 글로우 */
.p7-hudbar .p7-time-chip.p7-time-danger2 .p7-time-svg,
.p7-hudbar .p7-time-chip.p7-time-final2 .p7-time-svg{
  animation: p7_ring_glow .62s ease-in-out infinite !important;
}



/* =========================================================
   ✅ P7 TIME PRESSURE — ALWAYS ON (CLEAN OVERRIDE)
   - 기본(2분대)도 빨강/박동
   - 60/30/10초에서 더 강하게
   ========================================================= */

@keyframes p7_time_alive_red {
  0%   { transform: scale(1);   filter: brightness(1.05); }
  50%  { transform: scale(1.06); filter: brightness(1.35); }
  100% { transform: scale(1);   filter: brightness(1.05); }
}
@keyframes p7_time_warn_red {
  0%{transform:scale(1)} 40%{transform:scale(1.08)} 100%{transform:scale(1)}
}
@keyframes p7_time_danger_red {
  0%{transform:scale(1)} 20%{transform:scale(1.10)} 40%{transform:scale(.99)} 60%{transform:scale(1.12)} 100%{transform:scale(1)}
}
@keyframes p7_time_final_shake {
  0%{transform:translateX(0)} 25%{transform:translateX(-1px)} 50%{transform:translateX(1px)} 75%{transform:translateX(-1px)} 100%{transform:translateX(0)}
}
@keyframes p7_ring_glow {
  0%{filter:drop-shadow(0 0 6px rgba(255,60,60,.28))}
  50%{filter:drop-shadow(0 0 13px rgba(255,60,60,.70))}
  100%{filter:drop-shadow(0 0 6px rgba(255,60,60,.28))}
}

/* ✅ 기본 TIME: 빨강 숫자 + 숨쉬는 박동 */
.p7-hudbar .p7-time-chip{
  animation: p7_time_alive_red .80s ease-in-out infinite !important;
  background: rgba(0,0,0,0.34) !important;
  border: 1px solid rgba(255,90,90,0.40) !important;
  box-shadow:
    0 0 0 1px rgba(255,255,255,0.10) inset,
    0 0 28px rgba(255,60,60,0.22) !important;
}
...
.p7-hudbar .p7-time-chip b{
  color:#ff3b3b !important;
  letter-spacing: .06em !important;
}

/* 링 보이기 + 빨강 글로우 */
.p7-hudbar .p7-time-svg{ width:18px !important; height:18px !important; display:inline-block !important; }
.p7-hudbar .p7-time-svg circle{ opacity:1 !important; }
.p7-hudbar .p7-time-svg{ animation: p7_ring_glow .78s ease-in-out infinite !important; }

/* 60초↓: 더 빠르게 */
.p7-hudbar .p7-time-chip.p7-time-warn{
  animation: p7_time_warn_red .62s ease-in-out infinite !important;
  box-shadow: 0 0 0 1px rgba(255,204,0,.18) inset, 0 0 34px rgba(255,204,0,.18) !important;
}

/* 30초↓: 더 강하게 */
.p7-hudbar .p7-time-chip.p7-time-danger2{
  animation: p7_time_danger_red .50s ease-in-out infinite !important;
  border-color: rgba(255,60,60,0.60) !important;
  box-shadow: 0 0 0 1px rgba(255,60,60,.22) inset, 0 0 46px rgba(255,60,60,.34) !important;
}

/* 10초↓: TIME만 떨림 */
.p7-hudbar .p7-time-chip.p7-time-final2{
  animation: p7_time_final_shake .20s linear infinite !important;
  box-shadow: 0 0 0 1px rgba(255,40,40,.30) inset, 0 0 64px rgba(255,40,40,.48) !important;
}



/* =========================================================
   ✅ P7 TOP HUD TIME TUNING (VISIBLE + CLEAN)
   - TIME 단어: 검정
   - 숫자: 빨강 + tick
   - 배경: 어둡게 X (밝게)
   ========================================================= */
@keyframes p7TimeTickMini{
  0%{transform:translateY(0)}
  35%{transform:translateY(-0.6px)}
  70%{transform:translateY(0)}
  100%{transform:translateY(0)}
}
@keyframes p7TimeBreathMini{
  0%{transform:scale(1)}
...
.p7-hudbar .p7-time-chip{
  ...
  animation: p7TimeBreathMini 1.25s ease-in-out infinite !important;
}

/* "TIME" 단어는 검정(가독성) */
.p7-hudbar .p7-time-chip span{
  color:#111111 !important;
  font-weight:900 !important;
}
...
.p7-hudbar .p7-time-chip b{
  ...
  animation: p7TimeTickMini 1s steps(1) infinite !important;
}

/* 링도 조금 더 또렷 */
.p7-hudbar .p7-time-svg circle{
  opacity:1 !important;
}
...
/* =========================================================
   ✅ P7 BLUE TOKEN (FINAL OVERRIDE) — NO STRUCTURE CHANGE
   - 메인 허브(P7) 블루/시안 분위기와 전장 HUD/카드/선택지 통일
   - "추가"만 한다. 삭제/리팩터링 없음.
   ========================================================= */
:root{
  --p7-accent: #3CCBFF;          /* 시안 블루 */
  --p7-accent2: #4DA3FF;         /* 보조 블루 */
  --p7-accent-soft: rgba(60,203,255,.18);
  --p7-accent-soft2: rgba(77,163,255,.16);
}

/* (1) TOP HUD 바 + 칩 테두리: 파랑 톤 정렬 */
.p7-hudbar{
  border-color: rgba(60,203,255,.20) !important;
  box-shadow: 0 10px 26px rgba(0,0,0,0.22), 0 0 0 1px rgba(60,203,255,.10) inset !important;
}
.p7-chip{
  border-color: rgba(60,203,255,.22) !important;
}

/* (2) TIME 링: 기본은 P7 블루, warn/danger는 그대로 유지
   - ring_color가 SVG 속성(stroke)으로 들어가도 CSS !important로 덮어쓴다 */
.p7-time-svg circle:last-child{ stroke: var(--p7-accent) !important; }
...
/* (3) 지문 카드(인텔) 포인트 바 / 질문 카드(미션)도 P7 블루로 통일 */
.p7-zone.intel{
  background: linear-gradient(180deg, rgba(60,203,255,.10), rgba(60,203,255,.03)) !important;
  border-color: rgba(60,203,255,.24) !important;
}
.p7-zone.intel:before{
  background: rgba(60,203,255,.90) !important;
}
.p7-zone.mission{
  background: linear-gradient(180deg, rgba(77,163,255,.10), rgba(77,163,255,.03)) !important;
  border-color: rgba(77,163,255,.22) !important;
}
.p7-zone.mission:before{
  background: rgba(77,163,255,.90) !important;
}

/* (4) 선택지 hover(락온): P7 전장 전용 블루/시안 그라데이션 */
.p7-opt-wrap div[data-testid="stButton"] > button:hover{
  background: linear-gradient(135deg, var(--p7-accent), var(--p7-accent2)) !important;
  border-color: rgba(255,255,255,0.22) !important;
}
.p7-opt-wrap div[data-testid="stButton"] > button:hover,
.p7-opt-wrap div[data-testid="stButton"] > button:hover *{
...
/* (5) “선명한 P7 느낌” 한 방: 제목/핵심 강조에 아주 약한 시안 글로우 */
.p7-zone .p7-zone-body{
...
/* =========================================================
   ✅ P7 READABILITY FIX (DARK CARDS ON BLUE BG)
   - 구조/로직 0 변경, CSS만 추가
   - 지문/문제 카드만 어둡게 해서 글씨 대비를 즉시 살린다
   ========================================================= */

:root{
  --p7-card-bg: rgba(10, 16, 28, 0.72);
  --p7-card-bg2: rgba(12, 18, 34, 0.66);
  --p7-card-line: rgba(60, 203, 255, 0.26);
}

/* 지문/문제 카드: 어두운 유리판 */
.p7-zone{
  background: var(--p7-card-bg) !important;
  border: 1px solid var(--p7-card-line) !important;
  box-shadow: 0 14px 34px rgba(0,0,0,0.22) !important;
  backdrop-filter: blur(8px) !important;
}
.p7-zone.intel{ background: linear-gradient(180deg, rgba(10,16,28,0.78), rgba(10,16,28,0.66)) !important; }
.p7-zone.mission{ background: linear-gradient(180deg, rgba(12,18,34,0.78), rgba(12,18,34,0.66)) !important; }

/* 카드 안 글씨는 흰색 고정 + 선명한 그림자 */
.p7-zone .p7-zone-body{
  color: #ffffff !important;
  font-weight: 800 !important;
  opacity: 1 !important;
...
/* hover는 P7 블루 락온 */
.p7-opt-wrap div[data-testid="stButton"] > button:hover{
  background: linear-gradient(135deg, #3CCBFF, #4DA3FF) !important;
  color:#ffffff !important;
}
.p7-opt-wrap div[data-testid="stButton"] > button:hover *{
  color:#ffffff !important;
...
/* 모바일 글자 더 크게 (가독성 보장) */
@media (max-width: 640px){
  .p7-zone .p7-zone-body{ font-size: 18px !important; line-height: 1.6 !important; }
...
/* =========================================
   ✅ P7 OPTION TEXT BOLD FIX (SAFE)
   - 선택지 버튼 텍스트만 진하게 (색/크기/구조 변화 없음)
   ========================================= */
.p7-opt-wrap div[data-testid="stButton"] > button,
.p7-opt-wrap div[data-testid="stButton"] > button *{
  font-weight: 800 !important;
...
/* =========================================================
   ✅ P7 OPTION TEXT BOLD FORCE (FINAL)
   - 구조/로직 0 변경
   - 선택지(옵션) 텍스트만 "확실하게" 굵게(700)
   - 가장 마지막에 들어가서 무조건 이김(Last wins)
   ========================================================= */
...
/* === SNAPQ_P7_TEXTBOOST_BORDERPULSE_V1 === */
/* =========================================================
   P7 TEXT BOOST (SAFE) — keep card size, boost readability
   - only font/weight/shadow; no layout changes
   ========================================================= */
...
@media (max-width: 640px){
  ...
  .p7-zone.intel .p7-zone-body{ font-size: 19px !important; }
  .p7-opt-wrap div[data-testid="stButton"] > button{ font-size: 17px !important; }
}

/* =========================================================
   P7 BORDER PULSE (SAFE) — subtle arcade feel
   - no size change; only border/box-shadow animation
   ========================================================= */
@keyframes p7BorderPulse{
  0%{
    border-color: rgba(34,211,238,0.22);
    box-shadow: 0 14px 34px rgba(0,0,0,0.22), 0 0 0 1px rgba(34,211,238,0.10) inset;
  }
  50%{
    border-color: rgba(94,234,212,0.30);
    box-shadow: 0 14px 34px rgba(0,0,0,0.22), 0 0 0 1px rgba(94,234,212,0.14) inset;
  }
  100%{
    border-color: rgba(34,211,238,0.22);
    box-shadow: 0 14px 34px rgba(0,0,0,0.22), 0 0 0 1px rgba(34,211,238,0.10) inset;
...
/* Passage card: alive border */
.p7-zone.intel{ animation: p7BorderPulse 2.2s ease-in-out infinite; }

/* Question card: slightly softer */
.p7-zone.mission{ animation: p7BorderPulse 3.0s ease-in-out infinite; }


/* =========================================================
   ✅ P7 STEP BORDER (STATIC STRONG) — SAFE LOCK-IN
   - Step 1/2/3 마다 테두리 색이 "확실히" 다르게 보이게
   - 애니메이션 없음 (안전/눈 피로 방지)
   - 레이아웃/로직 변화 없음 (border 두께 대신 inset shadow 사용)
   ========================================================= */

/* p7-step가 붙은 카드만: 기존 펄스/애니메이션 차단 + 강한 색 고정 */
.p7-zone.p7-step{
  animation: none !important;

  /* border는 유지하되, 크기 안 바뀌게 'inset ring'으로 두께 표현 */
  border-color: rgba(255,255,255,0.14) !important;

  box-shadow:
    0 14px 34px rgba(0,0,0,0.22),
    0 0 0 1px rgba(255,255,255,0.06) inset,
    0 0 0 2px var(--p7-step-color) inset,
    0 0 18px var(--p7-step-glow) !important;
}

/* 좌측 포인트 바도 step 색으로 */
.p7-zone.p7-step:before{
  background: var(--p7-step-color) !important;
...
@media (max-width: 640px){
  .p7-zone.p7-step{
    box-shadow:
      ...
      0 0 0 1px rgba(255,255,255,0.06) inset,
      0 0 0 2px var(--p7-step-color) inset,
      0 0 24px var(--p7-step-glow) !important;
  }
}


/* =========================================================
   SNAPQ P7 TOP HUD: THICK GAUGE + HUB CHIP (SPACE-SAVER)
   - HUD bar thicker
   - gauge color shifts as time runs out
   ========================================================= */
.p7-hudbar{
...
.p7-hud-gauge .fill{
  ...
  width: 50%;
  border-radius: 999px;
  background: linear-gradient(90deg, rgba(34,211,238,0.90), rgba(167,139,250,0.90));
  transition: width .18s linear, background .18s linear;
}
.p7-hud-gauge.warn .fill{
  background: linear-gradient(90deg, rgba(255,204,0,0.92), rgba(249,115,22,0.90));
}
.p7-hud-gauge.danger .fill{
  background: linear-gradient(90deg, rgba(255,59,59,0.95), rgba(239,68,68,0.95));
}


/* =========================================================
   SNAPQ P7 HUD ULTRA COMPACT (v3)
   - MAIN HUB button ~1/4 size (icon-only)
   - reduce HUD height + gauge spacing
   - pull battlefield upward
   ========================================================= */
.block-container{
  padding-top: 0.10rem !important;
}
...
/* =========================================================
   P7 HUD v4 FINAL (SPACE-SAVER) — LAST WINS
   - All 5 elements look like one bar (STEP/TIME/MISS/COMBO/HUB)
   - HUB is a slim long pill
   - Gauge pulled up (more space for passage/questions/options)
   ========================================================= */
.p7-hudbar{
  margin: 0 0 4px 0 !important;
  padding: 4px 8px !important;
...
.p7-hud-left .p7-chip b{ font-size: 12px !important; }

/* MAIN HUB pill (scoped only to the right area) */
.p7-hud-right.p7-mainhub-compact div[data-testid="stButton"] > button{
...
.p7-hud-right.p7-mainhub-compact div[data-testid="stButton"] > button:hover{
  transform: translateY(-1px);
  background: rgba(255,255,255,1.0) !important;
}

/* Gauge: stick closer to HUD */
.p7-hud-gauge{
  height: 10px !important;
  margin: 2px auto 6px auto !important;
}

/* Pull battlefield upward a touch */
.p7-battlefield{ margin-top: 0px !important; }
...
def render_top_hud():
    """
    ✅ TOP HUD (one-line)
    - HTML이 글자로 보이지 않도록 textwrap.dedent + unsafe_allow_html=True 고정
    - MISS 2회: 심장박동+약한 흔들림, 3회: 더 강하게
    """
    update_timebomb()
    tb: TimebombState = st.session_state.p7_timebomb
    combo: ComboState = st.session_state.p7_combo
    ...
    miss_cls = "p7-chip" + (" p7-miss-danger" if wrong >= 2 else "")

    # ✅ C-1 오답 압박 오버레이
    if wrong >= 2:
        fx_cls = "p7-wrongfx " + ("w3" if wrong >= 3 else "w2")
        st.markdown(f"<div class='{fx_cls}'></div>", unsafe_allow_html=True)
    ...
    # TIME 단계
    is_warn = remaining <= 60
    is_danger = remaining <= 30
    is_final = remaining <= 10
    is_vignette = remaining <= 15

    time_cls = "p7-chip p7-time-chip p7-time-alive p7-exec-check"
    if is_warn:
    ...
    c = 2 * 3.14159265 * r
    dash = ratio * c
    ring_color = "#ff2d2d" if is_danger else ("#ffcc00" if is_warn else "rgba(255,255,255,0.90)")
    ring_svg = f"""
<svg class="p7-time-svg" viewBox="0 0 24 24" aria-hidden="true">
  <circle cx="12" cy="12" r="{r}" fill="none" stroke="rgba(255,255,255,0.18)" stroke-width="3"></circle>
...
"""
    ...
    if is_vignette:
        st.markdown("<div class='p7-vignette'></div>", unsafe_allow_html=True)
    
    
    # ---------------------------
    # TOP HUD (ONE-LINE) + THICK GAUGE (SPACE-SAVER)
    # - keeps passage space max (lots of English)
    # - Main Hub is a small "chip" inside the HUD bar (right)
    # - Gauge color shifts as time runs out
    # ---------------------------
    ...
    left_html = textwrap.dedent(
        ...
        <div class="p7-hud-left">
          ...
          <span class="{time_cls}">{ring_svg}<span>⏱ TIME <b>{get_time_display(remaining)}</b></span></span>
          <span class="{miss_cls}">❌ MISS <b>{wrong}</b>/3</span>
    ...
    colL, colR = st.columns([11, 1], gap="small")
    with colL:
        st.markdown(left_html, unsafe_allow_html=True)
    with colR:
        st.markdown('<div class="p7-hud-right p7-mainhub-compact">', unsafe_allow_html=True)
        if st.button("🏠 HUB", key="p7_hud_mainhub_btn", use_container_width=False):
            _safe_switch_page(
...
def render_p7_step(step: int):
    ...
    # --- Options: tap = submit (모바일 게임 방식) ---
    st.markdown("<div class='p7-opt-wrap'>", unsafe_allow_html=True)

    max_reached = int(st.session_state.get("p7_max_step_reached", step))
    for i, opt in enumerate(data.options_en):
        # NOTE: button 클릭 = 즉시 제출 (A안)
        if st.button(opt, use_container_width=True, key=f"p7_opt_{step}_{i}"):
            evaluate_step(step, i, data)

                                    # 정답이면 다음 스텝 / 오답이면 MISS 누적만 하고 계속
            if i == data.answer_index:
                if step < 3:
                ...
                # ✅ 3회 누적 오답이면 종료, 아니면 같은 Step 계속 도전
