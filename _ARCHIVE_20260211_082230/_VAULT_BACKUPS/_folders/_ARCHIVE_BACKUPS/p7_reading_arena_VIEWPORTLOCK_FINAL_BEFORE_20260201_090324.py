...
def inject_css():
    ...
    try:
        ...
        <div class="p7-force-badge">CSS FORCED ✅<br><b>P7_FORCE_TOP_OPTIONS_20260131_210415</b></div>
        """, unsafe_allow_html=True)
    except Exception:
        pass
    # ---- /P7_FORCE_TOP_OPTIONS_V1 ----
    ...
    # ---- P7_CSS_PROBE_V1 (AUTO, MUST BE VISIBLE) ----
    try:
        st.markdown(r"""
        <style>
        /* === P7_CSS_PROBE_V1 === */

        /* APPLY CHECK: thick border on container */
        .block-container{
          border: 4px solid rgba(0,255,170,0.55) !important;
          padding-top: 0rem !important;
          margin-top: 0rem !important;
        }

        /* KILL TOP GAP: pull battlefield upward (aggressive) */
        .p7-battlefield{ margin-top: -85px !important; padding-top: 0 !important; }
        .p7-zone{ margin-top: 6px !important; }

        /* OPTIONS: SUPER visible (even when disabled) */
        .p7-opt-wrap div[data-testid="stButton"]{ opacity: 1 !important; filter:none !important; }
        .p7-opt-wrap div[data-testid="stButton"] > button{
          background: #fff2a8 !important;   /* 노랑: 변화 확인용 */
          border: 2px solid #111 !important;
        }
        .p7-opt-wrap div[data-testid="stButton"] > button,
        .p7-opt-wrap div[data-testid="stButton"] > button * {
          font-size: 28px !important;      /* 극단값: 확인용 */
          font-weight: 950 !important;
          color: #000000 !important;
          opacity: 1 !important;
          -webkit-text-fill-color: #000000 !important;
        ...
        .p7-probe-badge b{ color:#00b894; }

        </style>

        <div class="p7-probe-badge">CSS PROBE ✅<br><b>P7_CSS_PROBE_20260131_222300</b></div>
        """, unsafe_allow_html=True)
    except Exception:
        pass
    # ---- /P7_CSS_PROBE_V1 ----
    ...
    try:
        ...
        <div style="position:fixed;right:12px;bottom:12px;z-index:999999;
                    padding:10px 12px;border-radius:14px;background:rgba(255,255,255,0.92);
                    border:3px solid rgba(0,255,170,0.70);font-weight:900;font-size:12px;
                    box-shadow:0 14px 30px rgba(0,0,0,0.25);">
          HARD FORCE ✅<br><span style="color:#00b894;">P7_HARD_FORCE_V1_20260131_223326</span>
        </div>
        """, unsafe_allow_html=True)
    except Exception:
        pass
    ...
    # ---- P7_FINAL_FORCE_V1 (AUTO, LAST WINS) ----
    try:
        st.markdown(r"""
        <style>
        /* === P7_FINAL_FORCE_V1 === */
        ...
        <div style="position:fixed;right:12px;bottom:12px;z-index:999999;
                    ...
                    border:3px solid rgba(0,255,170,0.70);font-weight:900;font-size:12px;
                    box-shadow:0 14px 30px rgba(0,0,0,0.25);">
          FINAL FORCE ✅<br><span style="color:#00b894;">P7_FINAL_FORCE_20260131_224000</span>
        </div>
        """, unsafe_allow_html=True)
    except Exception:
        pass
    ...
    # ---- P7_A_PLAN_FINAL_V2 (AUTO, LAST WINS) ----
    try:
        st.markdown("""
        <style>
        /* === P7_A_PLAN_FINAL_V2 === */
        ...
        <div style="position:fixed;right:12px;bottom:12px;z-index:999999;
                    ...
                    border:3px solid rgba(0,255,170,0.70);font-weight:900;font-size:12px;
                    box-shadow:0 14px 30px rgba(0,0,0,0.25);">
          A-PLAN ✅<br><span style="color:#00b894;">P7_A_PLAN_FINAL_V2_20260131_224731</span>
        </div>
        """, unsafe_allow_html=True)
    except Exception:
        pass
    ...
    # ---- TOPSPACE_FORCE_V9 (AUTO, LAST WINS) ----
    try:
        st.markdown("""
        <style>
        /* === TOPSPACE_FORCE_V9 === */
        ...
        div[data-testid="stAppViewContainer"] > .main{ padding-top:0 !important; margin-top:0 !important; }
        section.main > div{ padding-top:0 !important; margin-top:0 !important; }
        .block-container{ max-width:100% !important; padding-top:0 !important; margin-top:0 !important; padding-left:.25rem !important; padding-right:.25rem !important; }

        /* ⭐ 핵심: 극단값으로 확 당겨서 “체감” 먼저 만들기 */
        :root{ --p7-topshift: -160px; }  /* 나중에 -120 / -90으로 미세조정 */

        .p7-hud-gauge{ margin: 2px 0 4px 0 !important; }
        .p7-battlefield{ margin-top: var(--p7-topshift) !important; padding-top:0 !important; }

        /* 적용 증명 배지 */
        .p7-force-badge{
          position:fixed; right:12px; bottom:12px; z-index:999999;
          padding:10px 12px; border-radius:14px; background:rgba(255,255,255,0.92);
          border:3px solid rgba(0,255,170,0.70); color:#111827; font-weight:900; font-size:12px;
          box-shadow:0 14px 30px rgba(0,0,0,0.25);
        }
        .p7-force-badge b{ color:#00b894; }
        </style>
        <div class="p7-force-badge">TOPSPACE FORCE ✅<br><b>V9_20260131_234635</b></div>
        """, unsafe_allow_html=True)
    except Exception:
        pass
    # ---- /TOPSPACE_FORCE_V9 ----
...
try:
    st.markdown(r"""
    <style>
      /* 전장 자체를 위로 끌어올림 — Streamlit 무시 불가 */
      .p7-battlefield {
...
# ================== /P7_FORCE_TOP_KILL ==================
    ...
    # ---- P7_LAYOUT_V10 (AUTO, LAST WINS) ----
    try:
        st.markdown("""
        <style>
        /* === P7_LAYOUT_V10 === */

        /* 전장을 화면 높이로 쓰고(빈 공간 체감 제거) 내부를 flex로 배치 */
        .p7-battlefield{
          min-height: calc(100vh - 140px) !important;  /* HUD 높이 고려 */
          display: flex !important;
          flex-direction: column !important;
        }

        /* radiogroup(선택지 묶음)을 아래로 밀기: 남는 공간을 흡수 */
        .p7-battlefield div[role="radiogroup"]{
          margin-top: auto !important;
          padding-top: 10px !important;
        }

        /* 선택지 카드 간격 조금 안정화(너무 퍼지지 않게) */
        .p7-battlefield div[role="radiogroup"] > label{
          margin: 6px 0 !important;
        }
        ...
        <div class="p7-force-badge">LAYOUT ✅<br><b>V10_20260131_235925</b></div>
        """, unsafe_allow_html=True)
    except Exception:
        pass
    # ---- /P7_LAYOUT_V10 ----
...
# ===== P7_TOPSPACE_ULTRA_ZERO_BEGIN =====
try:
    st.markdown(r"""
    <style>
    /* =========================================================
       P7_TOPSPACE_ULTRA_ZERO_V1 (LAST WINS)
       - HUD 아래 여백 0px
       - battlefield 시작을 화면 최상단에 붙임
       - Streamlit 상단 패딩/마진/툴바/데코 강제 제거
       ========================================================= */
    ...
    /* block-container 완전 0 */
    .block-container {
      padding-top: 0 !important;
      margin-top: 0 !important;
    }

    /* 첫 블록(맨 위 element-container)이 공간 잡는 경우 차단 */
    .block-container > div:first-child {
      margin-top: 0 !important;
      padding-top: 0 !important;
    }
    ...
    /* HUD ↔ 전장 간격 0 */
    .p7-hudbar, .p7-hud-left, .p7-hud-gauge {
      margin-top: 0 !important;
      padding-top: 0 !important;
    }

    /* 전장 시작을 HUD 바로 아래로 "붙임" */
    .p7-battlefield {
      margin-top: 0 !important;
      padding-top: 0 !important;
    }
    ...
    pass
# ===== P7_TOPSPACE_ULTRA_ZERO_END =====





# ===== P7_TOPSPACE_FORCE_ZERO_V2_BEGIN =====
try:
    st.markdown(r"""
    <style>
    /* =========================================================
       P7 TOPSPACE FORCE ZERO v2
       - Kill FIRST element-container height
       ========================================================= */

    /* 첫 element-container 자체 무력화 */
    .block-container > div[data-testid="element-container"]:first-child {
    ...
    pass
# ===== P7_TOPSPACE_FORCE_ZERO_V2_END =====

# ===== P7_TOPPIN_A_PLAN_BEGIN =====
try:
    st.markdown(r"""
    <style>
    /* =========================================================
       P7 TOPPIN A-PLAN (SAFE)
       - Streamlit 상단 공간을 '삭제'하지 않는다.
       - HUD를 viewport top:0에 고정해서 '상단 빈 공간' 체감을 0으로 만든다.
       - Battlefield는 HUD 높이만큼만 아래로.
       ========================================================= */

    :root{
      /* HUD 높이(필요시 여기만 숫자 미세조정) */
      --p7-hud-h: 64px;
    }

    /* Streamlit 기본 상단 장식 제거(가능한 범위) */
    header[data-testid="stHeader"],
    div[data-testid="stToolbar"],
    div[data-testid="stDecoration"],
    ...
    /* HUD가 fixed라 본문이 가려지지 않도록, 전장은 HUD 높이만큼 아래에서 시작 */
    .p7-battlefield{
      margin-top: var(--p7-hud-h) !important;
      padding-top: 0 !important;
    }

    /* 혹시 첫 컨테이너가 높이를 잡아도 시각적으로 영향 최소화 */
    html, body { margin:0 !important; padding:0 !important; }
    </style>
    """, unsafe_allow_html=True)
except Exception:
    pass
# ===== P7_TOPPIN_A_PLAN_END =====
...
# ===== P7_HARDPIN_END =====
    ...
    try:
        ...
        <div class="p7-realfile-overlay">🔎 P7 REAL FILE: {REAL}</div>
    ...
    try:
        st.markdown(r"""
        <style>
        /* P7 AUTO-SCROLL + TOP PADDING ZERO (SAFE) */
        html, body { margin:0 !important; padding:0 !important; }
        .block-container { padding-top:0 !important; margin-top:0 !important; }
        div[data-testid="stAppViewContainer"], 
        div[data-testid="stAppViewContainer"] > section,
        ...
        /* 빈 element-container가 위 공간을 잡는 경우 최소화 */
        div[data-testid="element-container"]:empty { display:none !important; height:0 !important; margin:0 !important; padding:0 !important; }
        </style>

        <script>
        (function(){
          function go(){
            const hud = document.querySelector('.p7-hudbar');
            const bf  = document.querySelector('.p7-battlefield');
            const target = hud || bf;
            if(target){
              target.scrollIntoView({block:'start', inline:'nearest'});
              window.scrollBy(0,-2); // 아주 미세 보정
              return true;
            }
            return false;
          }
          // 즉시 1회 + 짧게 재시도(렌더 지연 대비)
          go();
          let n=0;
          const t=setInterval(()=>{
            n++;
    ...
    # ===== P7_VIEWPORT_LOCK_BEGIN =====
    try:
        st.markdown(r"""
        <style>
        /* =========================================================
           P7 VIEWPORT LOCK (GAME MODE)
           - BODY(문서) 스크롤을 죽여서 "아래 빈 배경" 자체를 없앰
           - 전장(.p7-battlefield) 안에서만 스크롤
           ========================================================= */

        :root{ --p7-hud-h: 64px; } /* 필요시 56~72만 조절 */
        ...
        /* 3) block-container도 뷰포트 고정(바깥 스크롤 차단) */
        .block-container{
          height:100vh !important;
          overflow:hidden !important;
          padding-top:0 !important;
        ...
        .p7-battlefield{
          ...
          padding-top: 0 !important;
          padding-bottom: 24px !important; /* 맨 아래 버튼 숨김 방지용 */
        }
        </style>
        """, unsafe_allow_html=True)
    except Exception:
        pass
    # ===== P7_VIEWPORT_LOCK_END =====
