...
/* ===== DISABLED_BY_SAFE_THEME__P7 ARENA VERIFY \+ FORCE v1 ===== */
...
/* ===== DISABLED_BY_SAFE_THEME__P7 ARENA STREAMLIT TARGET v2 (FONT 1.25 + CLEAN) ===== */
...
/* ===== DISABLED_BY_SAFE_THEME__P7 FINAL OVERRIDE v3 (GUARANTEED) ===== */
...
/* ===== DISABLED_BY_SAFE_THEME__P7 ARENA BATTLE THEME v1 (PC-BANG) ===== */
...
/* ===== DISABLED_BY_SAFE_THEME__P7 ARENA PC-BANG FORCE THEME v2 ===== */
:root{
  --p7bg1:#070B16;
  --p7bg2:#0B1220;
  --p7panel:#0F172A;
  --p7panel2:#020617;
  --p7line:rgba(148,163,184,0.18);
  --p7cyan:rgba(0,229,255,0.90);
  --p7blue:rgba(27,124,255,0.90);
  --p7text:rgba(255,255,255,0.96);
  --p7sub:rgba(226,232,240,0.86);
}

/* (A) PROOF watermark: must appear */
body::after{
  content:"P7_BATTLE_FORCED_v2";
  position:fixed;
  top:6px; right:10px;
  z-index:999999;
  font-size:12px;
...
/* (B) Force the page background to dark (Streamlit containers) */
html, body{
  background: radial-gradient(1100px 700px at 25% 0%, rgba(0,229,255,0.10), rgba(0,0,0,0)),
              radial-gradient(900px 600px at 80% 10%, rgba(27,124,255,0.10), rgba(0,0,0,0)),
              linear-gradient(180deg, var(--p7bg2), var(--p7bg1)) !important;
}
div[data-testid="stAppViewContainer"],
div[data-testid="stAppViewContainer"] > section,
div[data-testid="stAppViewContainer"] > section > main,
...
/* (C) Font: guaranteed 1.2x on all visible content in main */
section.main *{
  font-size: 120% !important;
}

/* (D) HUD text: bright & bold */
.p7-hud, .p7-hud *{
  color: var(--p7text) !important;
  font-weight: 950 !important;
  text-shadow: 0 2px 14px rgba(0,0,0,0.45) !important;
}

/* (E) Passage/Question blocks: target markdown containers directly */
div[data-testid="stMarkdownContainer"]{
  background: linear-gradient(180deg, rgba(15,23,42,0.92), rgba(2,6,23,0.92)) !important;
  border: 1px solid var(--p7line) !important;
  border-radius: 22px !important;
  padding: 18px 18px 18px 18px !important;
  margin-bottom: 14px !important;
  box-shadow: 0 0 18px rgba(56,189,248,0.10) !important;
}
div[data-testid="stMarkdownContainer"] *{
  color: var(--p7text) !important;
  font-weight: 900 !important;
...
/* Question 느낌: "두 번째 마크다운"이 질문인 경우가 많아서 약간 더 강하게 */
div[data-testid="stMarkdownContainer"]:nth-of-type(2){
  background: linear-gradient(180deg, rgba(2,6,23,0.96), rgba(2,6,23,0.90)) !important;
  border-color: rgba(0,229,255,0.24) !important;
  position: relative !important;
}
div[data-testid="stMarkdownContainer"]:nth-of-type(2)::before{
  content:"";
  display:block;
  height:2px;
  width:100%;
  margin-bottom:10px;
  background: linear-gradient(90deg, var(--p7cyan), var(--p7blue));
  opacity:0.95;
  border-radius:999px;
}

/* (F) Options: make them battle buttons (dark) */
div[role="radiogroup"] > label{
  background: linear-gradient(180deg, rgba(15,23,42,0.92), rgba(2,6,23,0.96)) !important;
  border: 1px solid rgba(148,163,184,0.22) !important;
  border-radius: 18px !important;
...
div[role="radiogroup"] > label:hover{
  border-color: rgba(0,229,255,0.38) !important;
  box-shadow: 0 0 0 1px rgba(0,229,255,0.10), 0 16px 38px rgba(0,0,0,0.30) !important;
  transform: translateY(-1px);
}
div[role="radiogroup"] > label:has(input:checked){
  border-color: rgba(0,229,255,0.78) !important;
...
/* ===== P7 ARENA SAFE THEME v1 (CLEAN + READABLE) ===== */

/* 0) Kill any debug watermark */
body::after{ display:none !important; }

/* 1) Font +20% (guaranteed) */
section.main *{
  font-size: 120% !important;
}
...
/* 3) Page stays BRIGHT (do not paint whole background dark) */
html, body{
...
/* 4) Passage + Question cards: navy, clean, with subtle neon strip */
div[data-testid="stMarkdownContainer"]{
...
div[data-testid="stMarkdownContainer"]::before{
  content:"";
  position:absolute;
  left:16px; right:16px; top:10px; height:2px;
  background: linear-gradient(90deg, rgba(0,229,255,0.65), rgba(27,124,255,0.65));
  opacity: 0.55;
  border-radius: 999px;
}
div[data-testid="stMarkdownContainer"] *{
...
/* 5) Options: KEEP WHITE (readability), add neon accent on hover/selected */
div[role="radiogroup"] > label{
  background: rgba(255,255,255,0.98) !important;
  border: 1px solid rgba(148,163,184,0.28) !important;
  border-radius: 18px !important;
...
div[role="radiogroup"] > label:hover{
  ...
  box-shadow: 0 0 0 1px rgba(0,229,255,0.10), 0 14px 34px rgba(0,0,0,0.14) !important;
}
div[role="radiogroup"] > label:has(input:checked){
  border-color: rgba(0,229,255,0.78) !important;
  box-shadow: 0 0 0 2px rgba(0,229,255,0.12), 0 16px 38px rgba(0,0,0,0.16) !important;
}

/* ===== P7 ARENA CLEAN WHITE RESTORE v1 (FINAL OVERRIDE) ===== */

/* 0) Hide any debug watermark that might exist */
body::after{ display:none !important; }

/* 1) Background: restore WHITE everywhere */
...
/* 2) Font: guaranteed 1.2x (safe) */
section.main *{
  font-size: 120% !important;
}

/* 3) HUD: readable white + bold (no structural change) */
.p7-hud, .p7-hud *{
  color: rgba(255,255,255,0.98) !important;
  font-weight: 950 !important;
  letter-spacing: 0.35px !important;
...
/* 4) Passage + Question: navy cards (clean) */
div[data-testid="stMarkdownContainer"]{
  background: linear-gradient(180deg, rgba(15,23,42,0.95), rgba(2,6,23,0.95)) !important;
  border: 1px solid rgba(148,163,184,0.18) !important;
  border-radius: 22px !important;
...
/* Optional: subtle strip (very light, not heavy) */
div[data-testid="stMarkdownContainer"]::before{
  content:"";
  position:absolute;
  left:16px; right:16px; top:10px; height:2px;
  background: linear-gradient(90deg, rgba(0,229,255,0.50), rgba(27,124,255,0.50));
  opacity: 0.35;
  border-radius: 999px;
}

/* 5) Options: restore WHITE cards (readability) */
div[role="radiogroup"] > label{
  background: rgba(255,255,255,0.98) !important;
  border: 1px solid rgba(148,163,184,0.28) !important;
  border-radius: 18px !important;
