# === P7_CUM_SENTENCES_V3 applied at 20260203_190928 ===
...
def render_p7_step(step: int):
    ...
    # === P7_CUM_SENTENCES_V3 (2문장 → 5문장 → 8문장 누적 표시) ===
    # step1(2) + step2(3) + step3(3) 누적 합산 -> 표시용 cum_sentences
...
cum_sentences = s1 + s2 + s3
    ...
    st.markdown(
        ...
        <div style='padding:10px 12px;margin:10px 0;border:3px solid #ff2d2d;border-radius:14px;background:rgba(255,45,45,.12);font-weight:900;font-size:18px;'>✅ P7 PASSAGE PROBE HIT: P7_BUILD_20260205_145812</div>\n<div class='p7-zone intel p7-step' style='--p7-step-color:{step_color}; --p7-step-glow:{step_glow};'>
          <div class='p7-zone-body p7-textboost' style="font-size:clamp(26px,5.4vw,38px);font-weight:900;line-height:1.65;letter-spacing:0.1px">{(" ".join(cum_sentences)).replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
