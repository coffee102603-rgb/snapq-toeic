...
def init_state():
    ...
    st.session_state.setdefault("p7_choice_nonce", 0)
    st.session_state.setdefault("p7_lobby_fuse_idx", 1)


def reset_state():
    # keep last fuse
    keep = int(st.session_state.get("p7_lobby_fuse_idx", 1))
    for k in list(st.session_state.keys()):
        if str(k).startswith("p7_"):
            del st.session_state[k]
    init_state()
    st.session_state["p7_lobby_fuse_idx"] = keep
...
<div class="p7-lobby-card">
  ...
  <div class="p7-lobby-sub">전장 속도(=세트 제한시간)를 고르고 바로 돌입하세요. HUD는 전투 시작 후에만 뜹니다.</div>
    ...
    nonce = int(st.session_state.get("p7_choice_nonce", 0))
    ...
    if choice is not None:
        ...
        st.session_state["p7_choice_nonce"] = int(st.session_state.get("p7_choice_nonce", 0)) + 1
        st.rerun()
