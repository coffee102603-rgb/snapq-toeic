...
# ============================================================
# SnapQ VOCA v1.0 (Rebuild)
# TRAIN: calm 1-card learning (no timer/HP/pressure)
# BATTLE: 44s fixed, 10Q, HP=3, no meaning reveal, big tap 2x2
# Data: auto-import from ANY secret_armory.json under project
# ============================================================
...
def _import_from_any_secret_armory_json() -> List[Card]:
...
def load_cards() -> List[Card]:
    ...
    # if empty -> import (always try)
    if len(cards) == 0:
        imported = _import_from_any_secret_armory_json()
        if imported:
            cards = imported
            save_cards(cards)
...
.kW{ font-size: 30px; font-weight: 950; margin: 2px 0 10px 0; }
...
.bigbtn div[data-testid="stButton"] > button{
  width:100% !important; min-height: 70px !important; border-radius: 18px !important;
...
.hud{ position: sticky; top: 10px; border-radius: 18px; padding: 12px 12px;
...
.choice div[data-testid="stButton"] > button{
  width:100% !important; min-height: 88px !important; border-radius: 18px !important;
...
# TRAIN
# -----------------------------
def train_filtered(cards: List[Card]) -> List[Card]:
...
def render_add_form(cards_all: List[Card]):
    # only shown when no cards (or user expands)
...
def render_train(cards_all: List[Card]):
    ...
    if not deck:
        st.error("표시할 무기가 없습니다. P7 저장 단어(secret_armory.json)가 비어있거나 못 찾았을 수 있어요.")
    ...
    st.markdown(f'<div class="kW">{cur.word}</div>', unsafe_allow_html=True)
    st.markdown('<div class="kL">CHUNK</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="kC">{cur.chunk_en or "(chunk 없음)"}</div>', unsafe_allow_html=True)
    st.markdown('<div class="kL">SENTENCE</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="kS">{cur.sentence_en or "(sentence 없음)"}</div>', unsafe_allow_html=True)

    if st.session_state.get("train_reveal", False):
        if cur.meaning_ko:
            st.markdown(f'<div class="kKO">뜻: {cur.meaning_ko}</div>', unsafe_allow_html=True)
    ...
    with b1:
        st.markdown('<div class="bigbtn">', unsafe_allow_html=True)
        if st.button("뜻 보기", use_container_width=True, key=f"reveal_{cur.word}"):
            st.session_state["train_reveal"] = not bool(st.session_state.get("train_reveal", False))
            cur.seen += 1
            cur.added_at = cur.added_at or _now()
            save_cards(_upsert(cards_all, cur))
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)

    with b2:
        st.markdown('<div class="bigbtn">', unsafe_allow_html=True)
        if st.button("다음", use_container_width=True, key=f"next_{cur.word}"):
            train_next(len(deck))
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with b3:
        st.markdown('<div class="bigbtn">', unsafe_allow_html=True)
        if st.button("폐기", use_container_width=True, key=f"del_{cur.word}"):
            save_cards(_delete(cards_all, cur.word))
            st.toast("폐기 완료", icon="🧹")
            st.session_state["train_idx"] = 0
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)

    # optional manual add (collapsed)
...
def render_battle(cards_all: List[Card]):
    ...
    with left:
        # timeout rule: no answer reveal, jump back to TRAIN
            ...
            with cols[i]:
                ...
                if st.button(ch, use_container_width=True, key=f"ch_{qi}_{i}"):
                    ...
                    if ch == ans:
                        ...
                        st.session_state["battle_hp"] = hp - 1
                        st.session_state["battle_locked"] = False
                        st.toast("MISS ❌", icon="❌")
                        st.rerun()
                st.markdown("</div>", unsafe_allow_html=True)
