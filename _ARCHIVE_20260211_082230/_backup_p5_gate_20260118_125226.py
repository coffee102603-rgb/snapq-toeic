...
# UI/CSS SAFE PATCH:
# - Center-align all texts (mobile friendly)
# - Add subtle lane tints: TIMER / GRAMMAR / VOCA
# - Tune P5 warm arena palette (brown+orange)
# - A-mode brightness uplift (mobile readability): +12~18% brighter, same mood
# - Keep: Grammar 4 buttons + auto-enter, VOCA 3 buttons + auto-enter
# - Hide Launch buttons inside expander (backup only)
...
def _inject_gate_css():
    st.markdown(
        ...
        :root{
          ...
          /* ✅ A-mode: overall readability lift */
          --line: rgba(255,255,255,0.20);   /* 0.16 -> 0.20 */
          --txt: rgba(255,255,255,0.96);    /* 0.94 -> 0.96 */
          --muted: rgba(255,255,255,0.86);  /* 0.82 -> 0.86 */

          /* warm + dark base (slightly brighter) */
          --sideA: rgba(80, 45, 25, 0.26);  /* was ~0.22 */
          --sideB: rgba(10, 10, 14, 0.24);  /* was ~0.28, but we brighten via bg */

          --chip-rule-a: rgba(180, 40, 70, 0.22);
          --chip-rule-b: rgba(255, 45, 85, 0.14);
          --chip-timer-a: rgba(255, 140, 0, 0.20);
          --chip-timer-b: rgba(255, 122, 24, 0.13);

          /* lane tints (a touch stronger for mobile separation) */
          --lane-timer-a: rgba(255, 122, 24, 0.18); /* 0.14 -> 0.18 */
          --lane-timer-b: rgba(255, 45, 85, 0.08);  /* 0.06 -> 0.08 */

          --lane-gram-a: rgba(255, 45, 85, 0.18);   /* 0.14 -> 0.18 */
          --lane-gram-b: rgba(255, 122, 24, 0.08);  /* 0.06 -> 0.08 */

          --lane-voca-a: rgba(167, 139, 250, 0.18); /* 0.14 -> 0.18 */
          --lane-voca-b: rgba(56, 189, 248, 0.08);  /* 0.06 -> 0.08 */
        }

        /* ✅ A-mode: background lifted (still warm/dark, but less "black") */
        [data-testid="stAppViewContainer"]{
          background:
            radial-gradient(900px 650px at 28% 18%, rgba(255, 122, 24, 0.22), transparent 58%),
            radial-gradient(820px 620px at 70% 14%, rgba(255, 45, 85, 0.20), transparent 60%),
            radial-gradient(900px 900px at 52% 65%, rgba(140, 80, 30, 0.22), transparent 70%),
            rgba(10,10,14,0.88) !important; /* 0.94 -> 0.88 */
        }
        ...
        /* ✅ Center-align EVERYTHING (text + chips + cards) */
        ...
        /* ✅ Top mini-nav: slightly brighter glass */
        .p5-topnav div[data-testid="stButton"] > button{
          height: 52px !important;
          font-size: 15px !important;
          font-weight: 950 !important;
          border-radius: 14px !important;
          background: rgba(255,255,255,0.13) !important; /* 0.11 -> 0.13 */
          border: 1px solid rgba(255,255,255,0.18) !important; /* 0.16 -> 0.18 */
          color: var(--txt) !important;
          box-shadow: 0 10px 22px rgba(0,0,0,0.20) !important;
        }

        /* ✅ Side panel slimmer tone + TIMER lane tint (brighter) */
        .p5-panel{
          border-radius: 18px;
          padding: 10px 10px;
          background:
            radial-gradient(700px 280px at 50% 15%, var(--lane-timer-a), transparent 62%),
            linear-gradient(180deg, var(--sideA), var(--sideB));
        ...
        /* ✅ A-mode: briefbar brighter for readability */
        .p5-briefbar{
          border-radius: 14px;
          padding: 10px 10px;
          background: rgba(255,255,255,0.13); /* 0.10 -> 0.13 */
          border: 1px solid rgba(255,255,255,0.16);
          color: var(--txt);
          font-weight: 950;
          line-height: 1.18;
        ...
        /* ✅ Timer buttons: keep bright + selected ring */
        ...
        .p5-selected{
          outline: 3px solid rgba(255,45,85,0.50);
          border-radius: 18px;
        ...
        /* ✅ Lanes (Grammar/Voca) subtle tints (slightly stronger) */
        .p5-lane{
          border-radius: 18px;
          padding: 0px; /* keep internal cards shape */
        }
        .p5-lane-gram{
          background:
        ...
        .p5-chip{
          ...
          padding: 8px 12px;
          border-radius: 999px;
          background: linear-gradient(90deg, rgba(255,45,85,0.22), rgba(255,122,24,0.16)); /* tiny lift */
        ...
        /* ✅ Choice buttons: mobile tap-friendly + allow wrapping */
        ...
        /* ✅ Current selection bar: slightly brighter so it doesn't look "heavy" */
        .p5-currentbar{
          margin-top: 8px;
          padding: 8px 10px;
          border-radius: 12px;
          background: rgba(255,255,255,0.10); /* 0.08 -> 0.10 */
        ...
        /* ✅ Expander (manual launch): brighter + quieter */
        details{
          border: 1px solid rgba(255,255,255,0.14) !important;
          border-radius: 14px !important;
          background: rgba(255,255,255,0.07) !important; /* 0.05 -> 0.07 */
        }
        details summary{
