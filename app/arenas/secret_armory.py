...
# ============================================================
# SnapQ VOCA v1.0 FINAL (TRAIN vs BATTLE)
# - TRAIN: calm 1-card learning (no timer/HP/pressure)
# - BATTLE: 44s fixed / 10Q / HP=3 / no meaning reveal / 2x2 big tap
# - FAIL: timeout/hp0 => no answer reveal, streak reset, force word back to TRAIN
# - Import: scan ANY secret_armory.json under project (newest first)
# ============================================================
...
def save_cards(cards: List[Card]) -> None:
    _safe_write_json(WORDS_JSON, {"schema":"secret_armory_v1","updated_at":_now(),"cards":[c.to_dict() for c in cards]})
...
def _import_p7_cards() -> List[Card]:
    for p in _find_secret_armory_json():
        ...
        for it in raw:
            ...
            cards.append(Card(word=w, meaning_ko=meaning, chunk_en=chunk, sentence_en=sent, tag=str(it.get("tag","P7")) or "P7", added_at=_now()))
        cards = _dedupe(cards)
        if cards:
...
.hero{ border-radius: 18px; padding: 14px 16px; border: 1px solid rgba(255,255,255,0.14);
  background: rgba(255,255,255,0.06); box-shadow: 0 12px 30px rgba(0,0,0,0.22); backdrop-filter: blur(10px); }
...
.badge{ display:inline-block; padding: 6px 10px; border-radius: 999px; border: 1px solid rgba(255,255,255,0.18);
  background: rgba(0,0,0,0.22); font-size: 12px; font-weight: 900; }

.card{ border-radius: 18px; padding: 16px 16px; border: 1px solid rgba(255,255,255,0.16);
  background: rgba(255,255,255,0.08); box-shadow: 0 16px 36px rgba(0,0,0,0.22); backdrop-filter: blur(10px); }
...
def ensure_state():
    ...
    st.session_state.setdefault("train_force_queue", [])
...
def render_add_form(cards_all: List[Card]):
    ...
    if st.button("✅ 무기 등록", use_container_width=True):
        ...
        if not w2:
            ...
            new = Card(word=w2, meaning_ko=(m or "").strip(), chunk_en=(ch or "").strip(), sentence_en=(se or "").strip(), tag=(tag or "MANUAL"), added_at=_now())
            save_cards(_upsert(cards_all, new))
            st.toast("등록 완료", icon="✅")
            st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)


def render_train(cards_all: List[Card]):
    ...
    with c2:
        ...
        st.session_state["train_tag"] = st.selectbox("🏷️ 태그", options=tags, index=(tags.index(cur) if cur in tags else 0), key="train_tag_in")

    deck = train_filtered(cards_all)
    st.markdown(f"<span class='badge'>총 무기:{len(cards_all)}</span> &nbsp; <span class='badge'>표시 중:{len(deck)}</span>", unsafe_allow_html=True)
    st.markdown("")
    ...
    curc = train_current(deck); assert curc is not None

    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.markdown("<div class='kL'>WORD</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='kW'>{curc.word}</div>", unsafe_allow_html=True)
    st.markdown("<div class='kL'>CHUNK</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='kC'>{curc.chunk_en or '(chunk 없음)'}</div>", unsafe_allow_html=True)
    st.markdown("<div class='kL'>SENTENCE</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='kS'>{curc.sentence_en or '(sentence 없음)'}</div>", unsafe_allow_html=True)

    if st.session_state.get("train_reveal", False) and curc.meaning_ko:
        st.markdown(f"<div class='kKO'>뜻: {curc.meaning_ko}</div>", unsafe_allow_html=True)
    ...
    with b1:
        st.markdown('<div class="bigbtn">', unsafe_allow_html=True)
        if st.button("뜻 보기", use_container_width=True, key=f"reveal_{curc.word}"):
            st.session_state["train_reveal"] = not bool(st.session_state.get("train_reveal", False))
            curc.seen += 1
            curc.added_at = curc.added_at or _now()
            save_cards(_upsert(cards_all, curc))
            _remove_force(curc.word)
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
    with b2:
        st.markdown('<div class="bigbtn">', unsafe_allow_html=True)
        if st.button("다음", use_container_width=True, key=f"next_{curc.word}"):
            _remove_force(curc.word)
            train_next(len(deck))
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)
    with b3:
        st.markdown('<div class="bigbtn">', unsafe_allow_html=True)
        if st.button("폐기", use_container_width=True, key=f"del_{curc.word}"):
            save_cards(_delete(cards_all, curc.word))
            _remove_force(curc.word)
            st.toast("폐기 완료", icon="🧹")
            st.session_state["train_idx"] = 0
            st.rerun()
...
def stage(sec: int) -> str:
    if sec >= 25: return "gray"
    if sec >= 11: return "orange"
    return "red"
...
def _fail(reason: str, target_word: Optional[str]):
...
def render_battle(cards_all: List[Card]):
    ...
    with left:
        if st.session_state.get("battle_running") and battle_left() <= 0:
            ...
            _fail("timeout", target)
            st.rerun()

        if not st.session_state.get("battle_running"):
            st.info("▶ START를 누르면 44초 심문실이 시작됩니다.")
            st.markdown("</div>", unsafe_allow_html=True)
            return

        qs = st.session_state.get("battle_questions", []) or []
        if not qs:
            qs = make_questions(cards_all)
            st.session_state["battle_questions"] = qs
        ...
        st.markdown(f"<span class='badge'>Mission {qi+1}/{BATTLE_Q}</span> &nbsp; <span class='badge'>⏱ {sec}s</span>", unsafe_allow_html=True)
        st.markdown("")
...
def app():
    ...
    mode = st.radio("모드 선택", ["TRAIN","BATTLE"],
                    index=0 if st.session_state.get("voca_mode","TRAIN")=="TRAIN" else 1,
                    horizontal=True, key="voca_mode_radio")
...
# ============================================================
# P5_ARMORY_COMPAT_PATCH_V1
# - pages/02_P5_Train_Arena.py, pages/02_P5_Armory_Battle.py compatibility
# - provides: _load_armory_items(), _save_armory_items(), _render_p5_timebomb(items)
# ============================================================
from pathlib import Path
import json
import streamlit as st

def _armory_items_path() -> Path:
...
def _load_armory_items():
    """Return list[dict] that includes both P5 and P7 items."""
    p = _armory_items_path()
    try:
...
def _save_armory_items(items):
    """Save list[dict] back to secret_armory.json (keeps P5/P7 together)."""
    p = _armory_items_path()
    try:
        p.parent.mkdir(parents=True, exist_ok=True)
...
def _render_p5_timebomb(items):
    """
    P5 Armory Battle renderer shim.
    Your project already has dedicated P5 arenas, so we provide a safe launcher UI here.
    """
    st.markdown("## 🔥 P5 ARMORY BATTLE")
    st.caption("호환 모드: 저장된 P5 문제로 전장 진입 (기존 P5 전장 페이지로 이동)")

    p5_items = [x for x in (items or []) if isinstance(x, dict) and x.get("source") == "P5"]
    st.info(f"저장된 P5 문제 수: {len(p5_items)}")

    c1, c2 = st.columns(2)
    with c1:
        if st.button("🧨 P5 TIMEBOMB 전장으로", use_container_width=True):
            try:
                st.switch_page("pages/02_P5_Timebomb_Arena.py")
            except Exception:
                st.warning("pages/02_P5_Timebomb_Arena.py 이동 실패")
    with c2:
        if st.button("🔥 P5 EXAM 10Q·44s로", use_container_width=True):
            try:
                st.switch_page("pages/02_P5_Exam_10Q_55S.py")
            except Exception:
                st.warning("pages/02_P5_Exam_10Q_55S.py 이동 실패")
