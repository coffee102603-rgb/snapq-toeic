...
# ============================================
# Timebomb / 콤보 관련
    # (옵션 A) END PANEL 토글 상태 — 진행 로직 영향 없음
...
@media (max-width: 640px){
  ...
  .p7-opt-wrap div[data-testid="stButton"] > button{
    ...
    st.success("MISSION CLEAR 🎁 — 완전 격파. ALL TARGETS DOWN.")

    st.markdown("""
    <style>
    /* === P7 END PANEL: HUD OFF + CONTRAST UP + SIZE DOWN (SAFE) === */
    .p7-hudbar, .p7-hud-gauge, .p7-hud-left, .p7-hud-right, .p7-hud, .p7-timer-wrap { display:none !important; }
.p7-end-card { font-size: 0.92em !important; }
.p7-end-card h1, .p7-end-card h2, .p7-end-card h3 { margin: 4px 0 !important; }
.p7-end-nav div[data-testid="stButton"]>button{ min-height: 44px !important; padding: 10px 12px !important; font-size: 15px !important; }
...
# - "8문장(=Step3 도달)" 이후에만 노출
# - 로직/세션키 변경 없음
# ==============================
try:
    reached_8 = int(st.session_state.get("p7_max_step_reached", 0)) >= 3 or any(getattr(_r, "step", 0) == 3 for _r in st.session_state.get("p7_results", []))
except Exception:
    reached_8 = False

if reached_8:
    # 간단 보상 계산(표시용) — 저장/누적은 하지 않음
    _results = st.session_state.get("p7_results", [])
    _correct = sum(1 for _r in _results if getattr(_r, "is_correct", False))
    _total = max(1, len(_results))
    _acc = _correct / _total

    # 남은 시간(가능할 때만)
    try:
        _tb = st.session_state.get("p7_timebomb", None)
        _remain = int(getattr(_tb, "remaining", 0)) if _tb is not None else 0
    except Exception:
        _remain = 0

    # 등급/보상(표시용)
    if _acc >= 1.0 and _remain >= 45:
        _rank, _xp, _coin = "S", 120, 60
    elif _acc >= 1.0:
        _rank, _xp, _coin = "A", 100, 50
    elif _acc >= 0.67:
        _rank, _xp, _coin = "B", 80, 40
    else:
        _rank, _xp, _coin = "C", 60, 30

    st.markdown(
        f"""
        <div class="p7-zone mission" style="border:1px solid rgba(34,211,238,0.28); background: linear-gradient(180deg, rgba(34,211,238,0.10), rgba(167,139,250,0.08));">
          <div class="p7-zone-body p7-textboost" style="font-size:clamp(22px,4.6vw,32px); font-weight:950; line-height:1.25;">
            🏁 <span style="color:rgba(34,211,238,0.95);">MISSION CLEAR 🎁</span><br><span style="font-weight:900;opacity:.95">완전 격파. ALL TARGETS DOWN.</span>
          </div>
          <div class="p7-zone-body" style="margin-top:10px; font-size:clamp(16px,3.8vw,20px); line-height:1.45; opacity:0.95;">
            전투 등급: <b style="font-size:1.15em;">{_rank}</b> &nbsp; | &nbsp;
            정확도: <b>{_correct}/{_total}</b> &nbsp; | &nbsp;
            남은 시간: <b>{_remain}s</b>
          </div>
          <div class="p7-zone-body" style="margin-top:10px; display:flex; gap:10px; flex-wrap:wrap;">
            <span style="padding:8px 12px; border-radius:999px; border:1px solid rgba(255,255,255,0.18); background:rgba(255,255,255,0.10);">⚡ XP +{_xp}</span>
            <span style="padding:8px 12px; border-radius:999px; border:1px solid rgba(255,255,255,0.18); background:rgba(255,255,255,0.10);">🪙 COIN +{_coin}</span>
            <span style="padding:8px 12px; border-radius:999px; border:1px solid rgba(255,255,255,0.18); background:rgba(255,255,255,0.10);">🎁 REWARD UNLOCKED</span>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    # --- Reward Panel (CLICKABLE) ---
    if st.button("🎁 보상 패널 열기 / 닫기", key="p7_reward_toggle"):
        st.session_state.p7_reward_open = not bool(st.session_state.get("p7_reward_open", False))

    if st.session_state.get("p7_reward_open", False):
        tA, tB, tC = st.tabs(["🈁 해석(전장 데이터)", "✅ 정답/해설", "🗂 단어/오답"])
        _results = st.session_state.get("p7_results", [])

        with tA:
            st.caption("※ 실제 해석 데이터가 있으면 여기에 표시됩니다. (현재는 영문 지문/문항을 그대로 공개)")
            # 가능한 키로 지문/해석을 찾아서 표시
            _passage = st.session_state.get("p7_current_passage_text", None)
            if _passage:
                st.write(_passage)
            else:
                # results 기반 fallback
                for i, r in enumerate(_results, start=1):
                    txt = getattr(r, "passage_text", "") or getattr(r, "passage", "")
                    if txt:
                        st.markdown(f"**Q{i} Passage**")
                        st.write(txt)

        with tB:
            st.caption("내 선택 vs 정답 — 결과 리스트 기반")
            if not _results:
                st.info("결과 데이터(p7_results)가 없습니다.")
            for i, r in enumerate(_results, start=1):
                q = getattr(r, "question", "") or getattr(r, "question_text", "")
                opts = getattr(r, "options", None) or getattr(r, "choices", None) or []
                picked = getattr(r, "picked_idx", None)
                ans = getattr(r, "answer_idx", None)
                ok = bool(getattr(r, "is_correct", False))
                st.markdown(f"**Q{i} · {'✅ CLEAR' if ok else '❌ MISS'}**")
                if q: st.write("Q:", q)
                if opts and picked is not None and 0 <= int(picked) < len(opts):
                    st.write("내 선택:", f"({chr(65+int(picked))}) {opts[int(picked)]}")
                if opts and ans is not None and 0 <= int(ans) < len(opts):
                    st.write("정답:", f"({chr(65+int(ans))}) {opts[int(ans)]}")
                st.divider()

        with tC:
            st.caption("단어/오답 연결(개인 무기고) — 현재는 표시만")
            wrong_words = st.session_state.get("secret_armory_words", []) or st.session_state.get("p7_wrong_words", [])
            if wrong_words:
                st.write(" / ".join(map(str, wrong_words[:120])))
                if len(wrong_words) > 120:
                    st.caption(f"... +{len(wrong_words)-120}개 더")
            else:
                st.info("누적된 오답 단어가 없습니다.")

    st.markdown('<div class="p7-end-nav">', unsafe_allow_html=True)
    n1, n2, n3, n4 = st.columns(4, gap="small")
    with n1:
        if st.button("📊 SCOREBOARD", use_container_width=True, key="p7_end_to_score"):
            _safe_switch_page(
                ["pages/04_Scoreboard.py", "pages/04_Score_Board.py", "pages/04_SCOREBOARD.py"],
                fallback_hint="스코어보드",
            )
    with n2:
        if st.button("🗡 P5 전장", use_container_width=True, key="p7_end_to_p5"):
            _safe_switch_page(
                ["pages/02_P5_Timebomb_Arena.py", "pages/02_P5_Timebomb_Arena_PATCHED.py", "pages/02_P5_Arena.py"],
                fallback_hint="P5 전장",
            )
    with n3:
        if st.button("🧠 무기고(VOCA)", use_container_width=True, key="p7_end_to_vocab"):
            _safe_switch_page(
                ["pages/03_Secret_Armory_Main.py", "pages/03_Secret_Armory.py"],
                fallback_hint="비밀 무기고",
            )
    with n4:
        if st.button("🏠 MAIN HUB", use_container_width=True, key="p7_end_to_hub"):
            _safe_switch_page(
                ["main_hub.py", "./main_hub.py", "pages/00_Main_Hub.py", "pages\\00_Main_Hub.py"],
                fallback_hint="메인 허브",
            )
