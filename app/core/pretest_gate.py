...
def _which_gate_needed(profile: dict):
    diagnosis = profile.get("diagnosis", {})
    surveys = profile.get("surveys", {})
    first_access_str = profile.get("first_access", "")
    if not surveys.get("day1"):
        return ("survey", "day1")
    if not diagnosis.get("day1"):
        return ("diagnosis", "day1")
    try:
        ...
        return None
    if elapsed >= 10:
        if not surveys.get("day10"):
            return ("survey", "day10")
        if not diagnosis.get("day10"):
            return ("diagnosis", "day10")
    if elapsed >= 20:
        if not surveys.get("day20"):
            return ("survey", "day20")
        if not diagnosis.get("day20"):
            return ("diagnosis", "day20")
    return None
...
.gate-header { background: linear-gradient(135deg,rgba(255,170,0,0.15),rgba(255,60,0,0.1)); border: 1px solid rgba(255,170,0,0.3); border-radius: 16px; padding: 16px 20px; margin-bottom: 1.2rem; }
.gate-title { font-size: 1.4rem; font-weight: 900; color: #ffaa00 !important; margin-bottom: 4px; }
.gate-sub { font-size: 0.85rem; color: rgba(255,255,255,0.6) !important; }
...
SURVEYS = {
    "day1": {
        "title": "⚔️ 전장 입장 전 — 전투력 체크!",
        "sub": "딱 5문항! 솔직하게 답해야 전장이 열린다",
        "questions": [
            {"id": "s1_q1", "text": "🎯 나의 현재 영어 전투력은?", "options": ["500점 미만", "500~650점", "650~750점", "750점 이상"]},
            {"id": "s1_q2", "text": "🏆 이번 달 정복 목표는?", "options": ["600점 달성", "700점 달성", "800점 달성", "900점 이상"]},
            {"id": "s1_q3", "text": "⏰ 하루 평균 훈련 시간은?", "options": ["30분 미만", "30분~1시간", "1~2시간", "2시간 이상"]},
            {"id": "s1_q4", "text": "📋 지금까지 토익 실전 출전 횟수는?", "options": ["없음", "1~2회", "3~5회", "6회 이상"]},
            {"id": "s1_q5", "text": "📱 주로 사용할 전투 기기는?", "options": ["스마트폰", "태블릿", "노트북/PC", "번갈아 사용"]}
        ]
    },
    "day10": {
        "title": "⚔️ Day 10 — 중간 전투 보고!",
        "sub": "딱 4문항! 지난 10일간 훈련 현황을 보고하라",
        "questions": [
            {"id": "s10_q1", "text": "📅 지난 10일간 SnapQ 평균 출격 횟수는?", "options": ["거의 안 함 (1~2회)", "가끔 (3~5회)", "자주 (6~8회)", "매일 (9회 이상)"]},
            {"id": "s10_q2", "text": "⚡ 지난 10일간 가장 많이 한 전장은?", "options": ["P5 전장", "P7 전장", "역전장", "비슷하게 했음"]},
            {"id": "s10_q3", "text": "⏱️ P5 전장에서 주로 선택한 타이머는?", "options": ["30초 (최강 압박)", "40초 (중간)", "50초 (여유)", "매번 달랐음"]},
            {"id": "s10_q4", "text": "🔥 현재 전투 의지는 처음과 비교하면?", "options": ["많이 떨어짐", "비슷함", "조금 올라감", "많이 올라감"]}
        ]
    },
    "day20": {
        "title": "⚔️ Day 20 — 최종 전투 보고!",
        "sub": "딱 4문항! 한 달 훈련 결과를 보고하라",
        "questions": [
            {"id": "s20_q1", "text": "💪 SnapQ 훈련 후 실력이 올랐다고 느끼나요?", "options": ["전혀 그렇지 않다", "별로 그렇지 않다", "약간 그렇다", "많이 그렇다"]},
            {"id": "s20_q2", "text": "🏆 가장 도움이 된 전장은?", "options": ["P5 타이머 전장", "P7 단계별 독해", "역전장 (오답 재도전)", "전체 비슷하게 도움됨"]},
            {"id": "s20_q3", "text": "📱 모바일로 훈련할 때 화면 크기가 영향을 줬나요?", "options": ["전혀 영향 없음", "약간 불편했음", "보통", "화면이 커서 더 편했음"]},
            {"id": "s20_q4", "text": "📣 이 훈련소를 전우에게 추천하고 싶나요?", "options": ["전혀 아님", "별로", "보통", "적극 추천"]}
        ]
    }
}
...
def _render_survey(day_key: str, student_id: str, cohort_month: str) -> None:
    _apply_css()
    survey = SURVEYS.get(day_key, {})
    questions = survey.get("questions", [])
    total_q = len(questions)

    st.markdown(f'<div class="gate-header"><div class="gate-title">{survey.get("title","")}</div><div class="gate-sub">{survey.get("sub","")}</div></div>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="progress-bar-wrap"><div class="progress-bar-fill" style="width:{pct}%;"></div></div><div style="font-size:0.78rem;color:rgba(255,255,255,0.4);text-align:right;margin-bottom:1rem;">{answered_count} / {total_q} 완료</div>', unsafe_allow_html=True)
    ...
    if not all_answered:
        st.warning(f"⚠️ {total_q - len(answers)}문항 더 답해야 다음으로 넘어갈 수 있어요!")
    if st.button("다음 → 전장 자격 테스트", disabled=not all_answered, key="survey_submit_btn"):
...
def _render_diagnosis(day_key: str, student_id: str, cohort_month: str) -> None:
    ...
    day_labels = {"day1": "Day 1", "day10": "Day 10", "day20": "Day 20"}
    ...
    st.markdown(f'<div class="gate-header"><div class="gate-title">📋 {label} 자격 테스트</div><div class="gate-sub">전장 입장 전 필수 · P5 {len(p5_questions)}문제 + P7 {len(p7_questions)}문제 = 총 {total_q}문제</div></div>', unsafe_allow_html=True)
    ...
    all_answered = len(answers) >= total_q
    if not all_answered:
        st.warning(f"⚠️ {total_q - len(answers)}문제 더 답변해야 전장에 입장할 수 있어요!")
    if st.button("⚔️ 전장 입장!", disabled=not all_answered, key="diag_submit_btn"):
...
def _render_result(day_key, student_id, cohort_month, p5_questions, p7_questions, answers):
    ...
    if "diagnosis" not in profile:
        profile["diagnosis"] = {}
    profile["diagnosis"][day_key] = {
        "date": date.today().isoformat(),
        "total_score": total_score,
        "p5_score": round(p5_correct / len(p5_questions) * 100) if p5_questions else 0,
        "p7_score": round(p7_correct / len(p7_questions) * 100) if p7_questions else 0,
    ...
    if total_score >= 85:
        ...
        msg = "💪 우수 전사! 전장에서 더 강해져라!"
    elif total_score >= 50:
        msg = "⚔️ 훈련이 필요하다! 전장에서 실력을 키워라!"
    else:
        msg = "🔥 지금부터 시작이다! 전장이 너를 기다린다!"

    st.markdown(f'<div class="result-box"><div class="result-score">{total_score}점</div><div class="result-label">{day_label} 자격 테스트 완료 · {p5_correct+p7_correct}/{total_q} 정답</div></div>', unsafe_allow_html=True)
    st.markdown(f"**{msg}**")
    st.markdown("---")
...
def require_pretest_gate() -> None:
    ...
    if not profile:
        ...
        gate = _which_gate_needed(profile)

    if gate is None:
        return

    gate_type, day_key = gate

    if gate_type == "survey":
        if st.session_state.get(f"survey_{day_key}_done"):
            st.rerun()
            return
        _render_survey(day_key, student_id, cohort_month)
        st.stop()

    if gate_type == "diagnosis":
        if st.session_state.get(f"diagnosis_{day_key}_done"):
            return
        _render_diagnosis(day_key, student_id, cohort_month)
        st.stop()
