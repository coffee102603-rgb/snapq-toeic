"""
FILE: main_hub.py
ROLE: ?ëÏ†Ñ?¨Î†πÎ∂Ä (Î©îÏù∏ ?àÎ∏å/Î°úÎπÑ) ???âÎÑ§???ëÏÜç, ?§ÌÉØ ?úÏãú, ?ÑÏû• ?†ÌÉù
PHASES: LOGIN ??LOBBY (?®Ïñ¥ ?¨Î°ú?òÏö©??Ïπ¥Îìú + ?ÑÏû• 3Ï¢??†ÌÉù)
DATA:   storage_data.json ??word_prison, devices, research_logs
LINKS:  02_Firepower.py (?îÎ†•?? | 03_POW_HQ.py (?¨Î°ú?¨Î†πÎ∂Ä) | 04_Decrypt_Op.py (?îÌò∏?¥ÎèÖ ?ëÏ†Ñ)
PAPERS: ?ºÎ¨∏D(rt_logs), ?ºÎ¨∏A(adp_logs) ?∞Í≤∞ ?àÎ∏å
EXTEND: P4 Ï≤?ùå?ÑÏû• ?∞Îèô ?àÏ†ï | Î™®Î∞î???∏Ïãú ?åÎ¶º ?àÏ†ï
"""
...
def _load_research_logs(nickname: str) -> list:
    """data/research_logs ?êÏÑú ?¥Î≤à??Î°úÍ∑∏ Î°úÎìú"""
...
def _daily_rates(logs: list, activity_type: str, days: int = 7) -> list:
    """ÏµúÍ∑º N?ºÍ∞Ñ ?ºÎ≥Ñ ?ïÎãµÎ•?Î¶¨Ïä§??Î∞òÌôò (?∞Ïù¥???ÜÎäî ?†Ï? None)"""
...
def _get_motivation(rate, logs, activity_type):
    """?ïÎãµÎ•??∞ÏÜç?ÅÏäπ/?§Îäò?¨Î? Í∏∞Î∞ò ?êÍ∑π Î©òÌä∏"""
    ...
    if today_count == 0:
        return "?ò¥  ?§Îäò ?ÑÏßÅ 0Î¨∏Ï†ú!"
    if rate is None:
        return "?î•  Ï≤??ÑÏ†Ñ???úÏûë?òÏûê!"
    if rate >= 90:
        return "?èÜ  ?ÅÏúÑ 5%! ?ÑÎ≤Ω??Í∞ÄÍπùÎã§"
    if rate >= 80:
        return "?î•  ?ÅÏúÑ 10% ?òÏ?!"
    if rate >= 70:
        if rising:
            return "?ìà  3???∞ÏÜç ?ÅÏäπ Ï§?"
        return "?í™  Ï°∞Í∏àÎß??? Î™©Ìëú 80%"
    if falling:
        return "?†Ô∏è  3???∞ÏÜç ?òÎùΩ! ÏßÄÍ∏??πÏû•"
    return "?†Ô∏è  Î≥µÏäµ???ÑÏöî?òÎã§!"

def _get_visit_count(nickname: str) -> int:
    """?¥Î≤à???ëÏÜç ?üÏàò (Ï∂úÏÑù?ºÏàò Í∏∞Î∞ò)"""
...
def _calc_stats(nickname: str):
    """P5 ?ïÎãµÎ•? P7 ?ïÎãµÎ•? ?¨Î°ú?¨Î†πÎ∂Ä P5/Î≥¥Ïπ¥ ?ïÎ≥µÎ•? ?ëÏÜç?úÍ∞Ñ Í≥ÑÏÇ∞"""
...
def _mini_bar(rate):
    """Í∞ÑÎã®??Î∞?Í∑∏Îûò??HTML"""
    if rate is None:
        return '<span style="color:#666;font-size:14px;">?∞Ïù¥???ÜÏùå</span>'
    filled = int(rate / 10)
    bar = ""
    for i in range(10):
        h = 6 + (i * 2)
...
# =========================================================
def load_css():
    # Î™®Î∞î??Î∑∞Ìè¨??Î©îÌ??úÍ∑∏ - ?¥Î??∞Ïóê???¨Î∞îÎ•??¨Í∏∞Î°??úÏãú
    ...
    /* transform?ºÎ°ú ?àÏù¥?ÑÏõÉ Î¨¥Ïãú?òÍ≥† ?ÑÎ°ú ?¥Îèô */
    .block-container {
    ...
    /* ?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê
       Î∞òÏùë?????úÎ∏îÎ¶?(768px ?¥Ìïò)
    ?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê */
...
# ?Ä?Ä Streamlit ?ÅÎã® ?¨Î∞± ?ÑÏ†Ñ ?úÍ±∞ (MutationObserverÎ°?ÏßÄ??Í∞êÏãú) ?Ä?Ä
...
# ?Ö‚òÖ??iOS Safari ?∏ÏÖò ?ÅÏÜç????localStorage???âÎÑ§???Ä??+ WebSocket ?µÏñº?ºÏù¥Î∏??Ö‚òÖ??
...
# ??viewport_width ?êÎèô ?òÏßë (?ºÎ¨∏ 07 KCI ?µÏã¨ ???†Ìñâ?∞Íµ¨ 0Í±?
...
# viewport_widthÎ•?devices storage???Ä??
try:
    ...
    if _vw_raw and not st.session_state.get("_vw_saved_" + nickname):
        ...
        if os.path.exists(_STORAGE_FILE_HUB):
            ...
            _st_vw = {}
        # devices Î¶¨Ïä§?∏Ïóê?????ôÏÉù Í∏∞Î°ù Ï∞æÏïÑ??viewport_width Ï∂îÍ?/?ÖÎç∞?¥Ìä∏
        ...
        if not _updated:
            ...
            _devs.append({
                ...
                "screen_width":   _vw_int,   # ?ÄÎ¶¨Í∞í (JS?êÏÑú ???ïÌôï???ªÏúº?§Î©¥ Ï∂îÍ?)
...
# hub-brand ?úÍ±∞ ???ÑÎûò compact topbarÎ°??µÌï©

# =========================================================
# 3Í∞??ÑÏû• Ïπ¥Îìú (SVG ?ºÏù∏Ï∞®Ìä∏ + ?§Ïõå???òÏù¥??
# =========================================================
import streamlit.components.v1 as _hc
...
_welcome_short = "?î• Ï≤´Ìåê ¬∑ Î∞îÎ°ú ?úÏûë!" if _is_first_check else f"?í• {_total_solved}Î¨∏Ï†ú ?ÑÎ£å!"

# ?Ä?Ä ?ëÏÜç ?üÏàò Í∏∞Î∞ò Î°úÌÖå?¥ÏÖò Î©òÌä∏ ?Ä?Ä
_visit = _get_visit_count(nickname)
_is_first = (p5_count == 0 and p7_count == 0)

# P5 Î°úÌÖå?¥ÏÖò Î©òÌä∏ (??/??/??)
_P5_ROT = [
    ("?úÌóò?•Ï? ?ÑÏüÅ?∞Îã§!", "?∏ÌïòÍ≤??àÏ? ÎßàÎùº!", "??30Ï¥?¬∑ 5Î¨∏Ï†ú ¬∑ Î¨∏Î≤ï ?çÎèÑ??"),
    ("?ÑÏßÅ???êÎ¶¨??", "?ÑÏüÅ?∞Ïóî ?ëÍ≥Ñ ?ÜÎã§!", "??Î¨∏Î≤ï¬∑?¥Ìúò ?çÎèÑ??"),
    ("Î¨∏Ïû• 1Í∞úÎèÑ Î™??ΩÏúºÎ©¥ÏÑú", "?úÌóò??Í∞?Í±¥Í??", "??30Ï¥?¬∑ 5Î¨∏Ï†ú ¬∑ ?ÄÎ¶¨Î©¥ ?àÎùΩ!"),
    ("?§Î†•?Ä ?∞Ïäµ??ÎßåÎì†??", "?§Îäò???ÑÎßùÏπòÎ©¥ ?¥Ïùº??Í∞ôÎã§!", "??30Ï¥?¬∑ 5Î¨∏Ï†ú ¬∑ ?ÑÎßùÏπòÏ? ÎßàÎùº!"),
    ("Îπ†Î•¥Í≤? ?ïÌôï?òÍ≤å!", "Í∑∏Í≤å ?ÑÎ???", "??30Ï¥?¬∑ 5Î¨∏Ï†ú ¬∑ ?ÑÎ≤Ω?òÍ≤å!"),
]
# P7 Î°úÌÖå?¥ÏÖò Î©òÌä∏
_P7_ROT = [
    ("Í≥†Îìù?? ?êÌïò??", "Í∑∏Îüº ÏßÄÍ∏?Ï¶ùÎ™Ö??", "?ìñ 60Ï¥?¬∑ 1ÏßÄÎ¨?¬∑ 3Î¨∏Ï†ú ¬∑ ?ÖÌï¥ ÏßëÏ§ë??"),
    ("?¥Ïö© ?¥Ìï¥ Î™??òÎ©¥", "???ÄÎ¶∞Îã§. ÏßÄÎ¨∏ÏùÑ ?ΩÏñ¥??", "?ìñ 60Ï¥?¬∑ 1ÏßÄÎ¨?¬∑ 3Î¨∏Ï†ú ¬∑ ?§Îäò ?ùÎÇ¥!"),
    ("Í≥†Îìù??", "ÎßêÎßå ?òÏ? ÎßêÍ≥† Ï¶ùÎ™Ö??", "?ìñ 60Ï¥?¬∑ 1ÏßÄÎ¨?¬∑ 3Î¨∏Ï†ú ¬∑ ?ΩÍ≥† ?ùÎÇ¥??"),
    ("ÏßÄÎ¨∏ÏùÑ ?ΩÏ? ?äÏúºÎ©?, "Ï∞çÎäî Í≤ÉÍ≥º ?§Î¶Ñ?ÜÎã§!", "?ìñ 60Ï¥?¬∑ 1ÏßÄÎ¨?¬∑ 3Î¨∏Ï†ú ¬∑ ÏßëÏ§ë?òÎùº!"),
    ("?ÖÌï¥???çÎèÑÍ∞Ä ?ÑÎãà??", "?¥Ìï¥Í∞Ä ?ÑÎ???", "?ìñ 60Ï¥?¬∑ 1ÏßÄÎ¨?¬∑ 3Î¨∏Ï†ú ¬∑ ?ÑÎ≤Ω ?¥Ìï¥!"),
]
# ?¨Î°ú?¨Î†πÎ∂Ä Î°úÌÖå?¥ÏÖò Î©òÌä∏
_ARM_ROT = [
    ("P5 ?¨Î°ú ¬∑ 33Ï¥?¬∑ 5Î¨∏Ï†ú", "P7 ?¨Î°ú ¬∑ 44Ï¥?¬∑ 10Î¨∏Ï†ú", "?? ?¨Î°ú ?¨Î¨∏! ?§Îãµ ?ïÎ≥µ! ??†Ñ ?∏ÏÉù!"),
    ("???Ä?∏ÎÇò?", "Î∞òÎ≥µÎßåÏù¥ ?¥Í∏∏?¥Îã§!", "?ó°Ô∏??§Ïöï Î™??òÎ©¥ ??†Ñ???ÜÎã§!"),
    ("?ÄÎ¶?Î¨∏Ï†úÍ∞Ä", "?àÎ? Í∏∞Îã§Î¶∞Îã§!", "?ó°Ô∏??¥Î≤à??Î∞òÎìú???ïÎ≥µ?òÎùº!"),
    ("?§Îãµ?Ä Í∏∞Ìöå??", "?§Ïãú Î≥¥Î©¥ Î∞òÎìú???àÎã§!", "?ó°Ô∏???†Ñ???úÏûë?Ä ÏßÄÍ∏àÏù¥??"),
    ("?¨Í∏∞?òÎ©¥ ?∏Ìïò??", "?òÏ?Îß??êÏàò?????§Î•∏??", "?ó°Ô∏??ùÍπåÏßÄ ?∏Ïõå??"),
]
...
# ?¨Îùº?¥Îìú1: ?ïÎãµÎ•?(?¨Ï†ë?? or ?åÍ∞úÎ©òÌä∏ (Ï≤´Ï†ë??
...
if _is_first:
    _sn = student_name
    _p5_s1_big  = f"???îÎ†•??
    _p5_s1_lbl  = "Î¨∏Î≤ï¬∑?¥Ìúò ?çÎèÑ??"
    _p7_s1_big  = f"?ì° ?îÌò∏?¥ÎèÖ ?ëÏ†Ñ"
    _p7_s1_lbl  = "?¥ÎèÖ ?ëÏ†Ñ ¬∑ ?îÌò∏Î•??Ä?¥Îùº!"
    _arm_s1_big = f"?? ?¨Î°ú?¨Î†πÎ∂Ä"
    _arm_s1_lbl = "?ÄÎ¶?Î¨∏Ï†úÍ∞Ä Î¨¥Í∏∞Í∞Ä ?úÎã§"
    _p5_s2_big  = "3Í∞ú‚Üë ?ùÏ°¥"
    _p5_s2_lbl  = "30Ï¥?¬∑ 5Î¨∏Ï†ú ¬∑ ÎØ∏Îßå ?ÑÎ©∏"
    _p7_s2_big  = "1?§Îãµ Ï¶âÏÇ¨"
    _p7_s2_lbl  = "60Ï¥?¬∑ 3Î¨∏Ï†ú ¬∑ ?úÍ∞ÑÏ¥àÍ≥º Ï¶âÏÇ¨"
    _arm_s2_big = "?Ä?•‚ÜíÎ∞òÎ≥µ"
    _arm_s2_lbl = "?ÄÎ¶?Î¨∏Ï†úÍ∞Ä Î¨¥Í∏∞Í∞Ä ?úÎã§"
    _p5_s3  = "??5Î¨∏Ï†ú ?úÎ∞î?¥Î≤å ¬∑ Ï¶âÏÇ¨Î£?
    _p7_s3  = "?ì° ?ÖÌï¥ ÏßÄÎ¨??ïÎ≥¥??¬∑ 1?§Îãµ Ï¶âÏÇ¨"
    _arm_s3 = "?? ?Ä????Î∞òÎ≥µ ???ÑÏ†Ñ?ïÎ≥µ!"
else:
    _sn = student_name
    _p5_s1_big  = f"{p5_rate}%{_p5_trend}" if p5_rate is not None else f"{_sn}! Ï≤??ÑÏ†Ñ"
    _p5_s1_lbl  = "7???ïÎãµÎ•?
    _p5_s2_big  = f"{p5_count}Î¨∏Ï†ú"
    _p5_s2_lbl  = f"?¥Î≤à??¬∑ {_p5_trend} Î≥Ä?? if _p5_trend.strip() not in ["", "??, " ??] else "?¥Î≤à???Ä??
    _p7_s1_big  = f"{p7_rate}%{_p7_trend}" if p7_rate is not None else f"{_sn}! Ï≤??ÑÏ†Ñ"
    _p7_s1_lbl  = "7???ïÎãµÎ•?
    _p7_s2_big  = f"{p7_count}Î¨∏Ï†ú"
    _p7_s2_lbl  = f"?¥Î≤à??¬∑ {_p7_trend} Î≥Ä?? if _p7_trend.strip() not in ["", "??, " ??] else "?¥Î≤à???Ä??
    _arm_s1_big = f"{arm_pending}Í∞? if arm_pending > 0 else "?ÑÎ≤Ω!"
    _arm_s1_lbl = "?î• ?§Îãµ ?ÑÏßÅ ?®Ïïò??" if arm_pending > 0 else "???§Îãµ ?úÎ°ú!"
    _arm_s2_big = f"{arm_p5}%" if arm_p5 is not None else "??
    _arm_s2_lbl = "?ïÎ≥µÎ•?
    if p5_rate is None:
        _p5_s3 = f"??{_sn}! Ï≤??ÑÏ†Ñ Í∏∞Îã§Î¶¨Îäî Ï§?"
    elif p5_rate >= 80:
        _p5_s3 = f"?èÜ {p5_rate}% ¬∑ ?ÅÏúÑ 10%! ??Í∏∞ÏÑ∏ ?†Ï???"
    elif p5_rate >= 60:
        _p5_s3 = f"??{p5_rate}% ??Î™©Ìëú 80% ¬∑ ÏßÄÍ∏??ÑÏ†Ñ!"
    else:
        _p5_s3 = f"?? {p5_rate}%?! ?¥Í≤å ?§Î†•?¥Ïïº? ?πÏû• ?§Ïñ¥?Ä!"
    if p7_rate is None:
        _p7_s3 = f"?ìñ {_sn}! Ï≤??ÖÌï¥ ?ÑÌà¨ ?úÏûë?¥Îùº!"
    elif p7_rate >= 80:
        _p7_s3 = f"?èÜ {p7_rate}% ¬∑ ?ÖÌï¥ ÎßàÏä§?? Í≥ÑÏÜç ?¨Î†§!"
    elif p7_rate >= 60:
        _p7_s3 = f"?ìñ {p7_rate}% ??ÏßÄÎ¨???ÍººÍºº???ΩÏñ¥??"
    else:
        _p7_s3 = f"?? {p7_rate}%?! ÏßÄÎ¨??ΩÍ∏¥ ??Í±∞Ïïº?!"
    if arm_pending == 0:
        _arm_s3 = "???§Îãµ ?úÎ°ú! ?ÑÎ≤Ω ?ïÎ≥µ! ÏßÑÏßú ?ÑÏÇ¨!"
    elif arm_pending <= 5:
        _arm_s3 = f"?î• {arm_pending}Í∞??®Ïïò?? Í±∞Ïùò ???îÏñ¥!"
    else:
        _arm_s3 = f"?? {arm_pending}Í∞??ìÏù¥??Ï§? ???ÑÎßùÏ≥?"
...
# ?? C??Í∑∏Î¶¨??Ïπ¥Îìú ???¨Î°ú?òÏö©??+ ?îÎ†•??+ ?îÌò∏?¥ÎèÖ + ?¨Î°ú?¨Î†πÎ∂Ä
# ??iframe???ÑÎ? ???§ÌÅ¨Î°??ÜÏù¥ ???îÎ©¥??
# ?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê??

# ?Ä?Ä ?®Í? Î≤ÑÌäº CSS (JS ?§Ìñâ ??Ï¶âÏãú ?®Í∏∞Í∏? ?Ä?Ä
st.markdown("""
<style>
/* ?§ÎπÑÍ≤åÏù¥???®Í? Î≤ÑÌäº ???îÎ©¥???àÎ? Î≥¥Ïù¥ÏßÄ ?äÍ≤å */
...
_pr_badge_txt = (f"?ö® ?àÏ∂ú ÏßÅÏ†Ñ {_pr_danger}Î™? if _pr_danger > 0
                 else (f"?†Ô∏è ?ÑÌóò {_pr_warn}Î™? if _pr_warn > 0
                       else ("" if _pr_total == 0
                             else f"?Üï ?†ÏûÖ {_pr_new}Î™?)))
...
# ?Ä?Ä P5 Ïπ¥Îìú ?¨Îùº?¥Îìú3 Î©òÌä∏ (??Ï§??ïÏ∂ï) ?Ä?Ä
...
# ?Ä?Ä ?µÍ≥Ñ ?úÏãúÍ∞??Ä?Ä
_p5_disp  = f"{p5_rate}%" if p5_rate is not None else "Ï≤??ÑÏ†Ñ!"
_p7_disp  = f"{p7_rate}%" if p7_rate is not None else "Ï≤??ÑÏ†Ñ!"
_arm_disp = f"?¨Î°ú {_pr_total}Î™? if _pr_total > 0 else "?¨Î°ú ?ÜÏùå"
...
if p5_rate is not None:
    _npc_p5_stat = f"P5 ?ïÎãµÎ•?{p5_rate}% ¬∑ ?ÑÏ†Å {p5_count}Î¨∏Ï†ú"
else:
    _npc_p5_stat = "?ÑÏßÅ Ï≤??ÑÏ†Ñ ??¬∑ P5 Î¨∏Î≤ï?¥Ìúò ?çÎèÑ??"
if p7_rate is not None:
    _npc_p7_stat = f"P7 ?ïÎãµÎ•?{p7_rate}% ¬∑ ?ÑÏ†Å {p7_count}Î¨∏Ï†ú"
else:
    _npc_p7_stat = "?ÑÏßÅ Ï≤??ÑÏ†Ñ ??¬∑ P7 ?ÖÌï¥ ÏßÄÎ¨??¥ÎèÖ??"
if arm_p5 is not None:
    _npc_pow_stat = f"?§Îãµ {arm_pending}Í∞??òÍ∞ê ¬∑ ?ïÎ≥µÎ•?{arm_p5}%"
else:
    _npc_pow_stat = f"?§Îãµ {arm_pending}Í∞??òÍ∞êÏ§?
if _pr_total > 0:
    _npc_pb_stat = f"Ï¥?{_pr_total}Í∞?¬∑ P5:{_pr_p5} P7:{_pr_p7}"
else:
    _npc_pb_stat = "?¨Î°ú ?ÜÏùå! ?ÑÎ≤Ω ?ïÎ≥µ Ï§?
_npc_att_stat = f"Ï∂úÏÑù {att_days}??¬∑ Ï∞∏Ïó¨??{att_rate}%"
_GRID_HTML = f"""
<style>
*{{box-sizing:border-box;margin:0;padding:0;font-family:-apple-system,'Noto Sans KR',sans-serif;}}

/* ?Ä?Ä ?∞Í≤å??HUD ?ÅÎã®Î∞??Ä?Ä */
...
<!-- ??HUD ?ÅÎã®Î∞?-->
<div class="topbar">
  <img class="tb-photo" src="data:image/jpeg;base64,{TEACHER_B64}" alt="ÏµúÏÉò">
  <span class="tb-brand">??SNAPQ</span>
  <span class="tb-sep">|</span>
  <span class="tb-name">?ë§ {student_name}</span>
  <span class="tb-sep">|</span>
  <span class="tb-item">?ìÖ {att_days}??/span>
  <span class="tb-sep">|</span>
  <span class="tb-item">??{total_time}</span>
  <span class="tb-sep">|</span>
  <span class="tb-rank">?î• {ranking}</span>
  <span class="tb-sep">|</span>
...
<div class="pb"
  ...
  ontouchstart="">
<div class="npc-ov" style="border-radius:14px;"><div class="npc-sk">??</div><div class="npc-tx">?ÄÎ¶??®Ïñ¥?§Ïù¥ ?¨Í∏∞ Í∞áÌòî??<br>3Î≤??∞ÏÜç ÎßûÌ????ùÎ∞©.<br>Î™®Î•¥Î©??âÏÉù ?¨Í∏∞??</div></div>
<div class="npc-ov" id="ov-pb" style="border-radius:14px;"><div class="npc-sk">??</div><div class="npc-tx">?ÄÎ¶??®Ïñ¥?§Ïù¥ ?¨Í∏∞ Í∞áÌòî??<br>3Î≤??∞ÏÜç ÎßûÌ????ùÎ∞©.<br>Î™®Î•¥Î©??âÏÉù ?¨Í∏∞??</div><div class="npc-stat">{_npc_pb_stat}</div></div>
  <div class="pb-left">
    <div class="pb-icon">??</div>
    <div>
      <div class="pb-name">?®Ïñ¥ ?¨Î°ú?òÏö©??/div>
      <div style="font-size:9px;color:#aa88dd;margin-top:1px;font-weight:600;">?ÄÎ¶??®Ïñ¥Î•?Í∞Ä?êÍ≥† ¬∑ ?∏Ïö∏ ?åÍπåÏßÄ ?ùÎ∞© Í∏àÏ?!</div>
      <div class="pb-sub">{_pr_badge_txt}</div>
      <div style="font-size:10px;margin-top:3px;">
        <span style="color:#ff8833;font-weight:700;">???îÎ†•??{_pr_p5}Î™?/span>
        <span style="color:#445566;margin:0 4px;">¬∑</span>
        <span style="color:#00ccee;font-weight:700;">?ì° ?îÌò∏?¥ÎèÖ {_pr_p7}Î™?/span>
      </div>
    </div>
  </div>
  <div class="pb-btn">?¨Î¨∏ ??/div>
</div>
...
<div class="grid">
  ...
  <div class="card card-p5"
    ...
    ontouchstart="">
<div class="npc-ov"><div class="npc-sk">??/div><div class="npc-tx">P5???çÎèÑÍ∞Ä ?ÑÎ???<br>5Î¨∏Ï†ú, 3Í∞??ùÏ°¥.<br>Î∂àÏ≤ò??Î∞Ä?¥Î∂ô??</div></div>
<div class="npc-ov" id="ov-p5"><div class="npc-sk">??/div><div class="npc-tx">P5???çÎèÑÍ∞Ä ?ÑÎ???<br>5Î¨∏Ï†ú, 3Í∞??ùÏ°¥.<br>Î∂àÏ≤ò??Î∞Ä?¥Î∂ô??</div><div class="npc-stat">{_npc_p5_stat}</div></div>
    <div class="card-body">
      <div class="badge">Î¨∏Î≤ï¬∑?¥Ìúò</div>
      <div class="card-icon">??/div>
      <div class="card-name">?îÎ†•??/div>
      <div class="card-en">FIREPOWER</div>
      <div class="card-tags">
        <span class="tag">Î¨∏Î≤ï</span>
        <span class="tag">?¥Ìúò</span>
        <span class="tag">5Î¨∏Ï†ú</span>
      </div>
      <div class="card-rule">?? 3Í∞ú‚Üë ?ùÏ°¥ ¬∑ ÎØ∏Îßå ?ÑÎ©∏</div>
    </div>
    <div class="go-bar">
      <div class="go-stat">{_p5_disp}</div>
      <div class="go-btn">Ï∂úÍ≤© ??/div>
    </div>
  </div>
  ...
  <div class="card card-p7"
    ...
    ontouchstart="">
<div class="npc-ov"><div class="npc-sk">?ì°</div><div class="npc-tx">P7 ÏßÄÎ¨??¥ÎèÖ ?ÑÎ¨¥.<br>??1Î≤??§Ìåê = Ï¶âÏãú Ï≤†Ïàò.<br>ÏßëÏ§ë??</div></div>
<div class="npc-ov" id="ov-p7"><div class="npc-sk">?ì°</div><div class="npc-tx">P7 ÏßÄÎ¨??¥ÎèÖ ?ÑÎ¨¥.<br>??1Î≤??§Ìåê = Ï¶âÏãú Ï≤†Ïàò.<br>ÏßëÏ§ë??</div><div class="npc-stat">{_npc_p7_stat}</div></div>
    <div class="card-body">
      <div class="badge">?ÖÌï¥</div>
      <div class="card-icon">?ì°</div>
      <div class="card-name">?îÌò∏?¥ÎèÖ</div>
      <div class="card-en">DECRYPT OP</div>
      <div class="card-tags">
        <span class="tag">?¥ÎèÖ</span>
        <span class="tag">3Î¨∏Ï†ú</span>
        <span class="tag">Ï¶âÏÇ¨</span>
      </div>
      <div class="card-rule">???§Ìåê 1??= Ï¶âÏãú Ï≤†Ïàò</div>
    </div>
    <div class="go-bar">
  ...
  <div class="card card-pow"
    ...
    ontouchstart="">
<div class="npc-ov"><div class="npc-sk">??</div><div class="npc-tx">?§Í? ?ÄÎ¶?Î¨∏Ï†ú?§Ïù¥<br>?¨Í∏∞ Í∞áÌòî??<br>?ÑÏ†Ñ???∏Ïö∏ ?åÍπåÏßÄ ?ùÎ∞© ?ÜÏùå.</div></div>
<div class="npc-ov" id="ov-pow"><div class="npc-sk">??</div><div class="npc-tx">?§Í? ?ÄÎ¶?Î¨∏Ï†ú?§Ïù¥<br>?¨Í∏∞ Í∞áÌòî??<br>?ÑÏ†Ñ???∏Ïö∏ ?åÍπåÏßÄ ?ùÎ∞© ?ÜÏùå.</div><div class="npc-stat">{_npc_pow_stat}</div></div>
    <div class="pow-left">
      <div class="badge" style="background:rgba(255,255,255,0.15);">BOSS STAGE</div>
      <div style="font-size:22px;margin:4px 0 2px;">??</div>
      <div class="card-name">?¨Î°ú?¨Î†πÎ∂Ä</div>
      <div class="card-en">POW HEADQUARTERS</div>
      <div style="font-size:9px;color:rgba(255,255,255,0.5);margin:2px 0 4px;font-weight:600;">?? ?ÄÎ¶?Î¨∏Ï†úÍ∞Ä Î¨¥Í∏∞Í∞Ä ?úÎã§</div>
      <div class="card-tags">
        <span class="tag">?ÄÎ¶?Î¨∏Ï†ú Î≥µÏäµ</span>
        <span class="tag">?®Ïñ¥ ?¥Ï¶à</span>
        <span class="tag">?ÑÏ†Ñ ?ïÎ≥µ</span>
      </div>
    </div>
    <div class="pow-right">
      <div>
        <div class="pow-n">{_pr_total}</div>
        <div class="pow-l">?¨Î°ú ?òÍ∞êÏ§?/div>
      </div>
      <div class="pow-go">?ÖÏû• ??/div>
    </div>
  </div>
...
<div class="row">
  <div class="box box-p4">
    <div class="ico">?éµ</div>
    <div class="lbl">P4 Coming Soon</div>
  </div>
  <div class="box box-adm" onclick="goAdmin()">
    <div class="ico">?îí</div>
    <div class="lbl lbl-adm">Í¥ÄÎ¶¨Ïûê ?ÑÏö©</div>
  </div>
</div>
<script>
function goAdmin(){
...
function hideBtn(){
  try {
    ...
    // iOS?????¥Ï†Ñ Î¶¨Ïä§???úÍ±∞ ??Îß§Î≤à ?àÎ°ú ?±Î°ù (???¨ÏßÑ???úÏóê???ïÏÉÅ ?ôÏûë)
