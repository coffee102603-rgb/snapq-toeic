"""
SnapQ TOEIC V2 - Main Hub (???îÏûê??
C?? ?ÅÎã®Î∞∞ÎÑà + P5/P7 ?òÎ???+ ??†Ñ???ÑÏ≤¥ + ?òÎã® ?úÎ∏åÎ©îÎâ¥
"""
...
# ?Ö‚òÖ??iOS Safari ?∏ÏÖò ?ÅÏÜç????localStorage???âÎÑ§???Ä??+ WebSocket ?µÏñº?ºÏù¥Î∏??Ö‚òÖ??import streamlit.components.v1 as _ios_persist_cmp
_ios_nick_js = str(nickname).replace("'", "\\'") if nickname else ""
_ios_persist_cmp.html(f"""<script>
(function(){{
    // 1. localStorage???âÎÑ§???Ä??(WebSocket ?äÍ≤®??Î≥µÏõê Í∞Ä??
    try {{
        if('{_ios_nick_js}') {{
    ...
    // 2. WebSocket ?µÏñº?ºÏù¥Î∏???iOSÍ∞Ä Î∞±Í∑∏?ºÏö¥?úÏóê???∞Í≤∞???äÎäî Í≤?Î∞©Ï?
    // 30Ï¥àÎßà??Îπ?Î©îÏãúÏßÄÎ•?Î≥¥ÎÇ¥???∞Í≤∞ ?†Ï?
    if(!window._snapq_keepalive) {{
        window._snapq_keepalive = setInterval(function() {{
            try {{
                var ws = window.parent.document.querySelector('iframe');
                // Streamlit??WebSocket??ÏßÅÏ†ë ?ëÍ∑º?òÎäî ?Ä??                // visibility change ?¥Î≤§?∏Î°ú ?¨Ïó∞Í≤??∏Î¶¨Í±?            }} catch(e) {{}}
        }}, 25000);

        // ?îÎ©¥???§Ïãú Î≥¥Ïùº ??(iOS ???ÑÌôò ??Î≥µÍ?) ?∏ÏÖò Î≥µÏõê ?úÎèÑ
            ...
            if(document.visibilityState === 'visible') {{
                try {{
                    ...
                    if(nick && ag === '1') {{
                        ...
                        // ?ÑÏû¨ URL??Î©îÏù∏?¥Í≥† nick ?åÎùºÎØ∏ÌÑ∞Í∞Ä ?ÜÏúºÎ©?Ï∂îÍ?
...
# viewport_widthÎ•?devices storage???Ä??try:
...
<div class="top-banner">
    ...
    <span class="banner-item">?ìÖ Ï∂úÏÑù {att_days}??/span>
    <span class="banner-divider">|</span>
    <span class="banner-item">??{total_time}</span>
    <span class="banner-divider">|</span>
    <span class="banner-rank">?î• ?ÑÌà¨Ï∞∏Ïó¨??{ranking}</span>
</div>
""", unsafe_allow_html=True)
...
<div class="hub-brand">
    ...
    <div class="hub-brand-text">
        ...
        <div class="hub-subtitle">by Crazy ÏµúÏÉò ¬∑ HACKERS</div>
        <div class="hub-platform">BATTLE LOBBY ¬∑ ?ÑÏû•???†ÌÉù?òÏÑ∏??/div>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div class="wel">
  <div class="wt">?ëã Î∞òÍ??åÏöî, {student_name}??</div>
  <div class="ws">?î• Ï≤´Ìåê ¬∑ ÏßÄÍ∏?Î∞îÎ°ú ?úÏûë!</div>
</div>""", height=36)
else:
...
<div class="wel">
  <div class="wt">?î• ?§Ïãú ?îÍµ∞?? {student_name}??</div>
  <div class="ws">?í• {_total}Î¨∏Ï†ú ?ÑÎ£å ¬∑ ?§Îäò????????</div>
</div>""", height=36)
...
# ??†Ñ??Î°úÌÖå?¥ÏÖò Î©òÌä∏
_ARM_ROT = [
    ("Î∞òÎ≥µ P5Î¨∏Ï†ú ¬∑ 33Ï¥?¬∑ 5Î¨∏Ï†ú", "Î∞òÎ≥µ P7?¥Ìúò ¬∑ 44Ï¥?¬∑ 10Î¨∏Ï†ú", "?ó°Ô∏??§Îãµ ?§Ïöï?? ?†Ïùµ ??†Ñ! ?∏ÏÉù ??†Ñ!"),
...
if _is_first:
    ...
    _p7_s1_big  = f"?ìñ P7?ÑÏû•"
    _p7_s1_lbl  = "?ÖÌï¥ ¬∑ ?ΩÎäî ?åÎ? Íπ®Ïõå??"
    _arm_s1_big = f"?î• ?§Îãµ?ÑÏû•"
    ...
    _p5_s3  = "??5Î¨∏Ï†ú Ï§?3Í∞ú‚Üë ÎßûÏ∂∞???ùÏ°¥!"
    _p7_s3  = "?ìñ ?§Îãµ 1Í∞?Ï¶âÏÇ¨ ¬∑ ?úÍ∞ÑÏ¥àÍ≥º Ï¶âÏÇ¨"
    _arm_s3 = "?î• ?Ä????Î∞òÎ≥µ ???ÑÏ†Ñ ?ïÎ≥µ!"
else:
    _sn = student_name
...
# ?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê??# ?? ?®Ïñ¥ ?¨Î°ú?òÏö©???åÌòï Ïπ¥Îìú (?ÑÏû• Ïπ¥Îìú ??
# ?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê??try:
...
if _pr_total > 0:
    _pr_badge = f"?ö® ?àÏ∂ú ÏßÅÏ†Ñ {_pr_danger}Î™? if _pr_danger > 0 else (f"?†Ô∏è ?ÑÌóò {_pr_warn}Î™? if _pr_warn > 0 else f"?Üï ?†ÏûÖ {_pr_new}Î™?)
...
<div style="background:linear-gradient(135deg,#1a0a2e,#2d1060);
     ...
     ontouchstart="">
  <div>
    <span style="font-size:1.1rem;font-weight:900;color:#e8d0ff;">?? ?®Ïñ¥ ?¨Î°ú?òÏö©??/span>
    <span style="font-size:0.85rem;color:{_pr_color};margin-left:10px;font-weight:700;">{_pr_badge}</span>
  </div>
  <div style="display:flex;align-items:center;gap:12px;">
    <span style="font-size:1.4rem;font-weight:900;color:#fff;">?¨Î°ú {_pr_total}Î™?/span>
    <span style="background:{_pr_color};color:#fff;font-size:0.85rem;font-weight:700;
                 padding:4px 12px;border-radius:20px;">?¨Î¨∏?òÍ∏∞ ??/span>
  </div>
</div>"""
    ...
    if _prison_go:
        ...
        st.switch_page("pages/03_?§Îãµ?ÑÏû•.py")
else:
    _pr_empty_html = """
<div style="background:linear-gradient(135deg,#0a1a0a,#1a2a1a);
  ...
  <span style="font-size:0.9rem;color:#50c870;font-weight:700;">
    ?? ?®Ïñ¥ ?¨Î°ú?òÏö©?????¨Î°ú ?ÜÏùå! ?ÑÎ≤Ω ?ïÎ≥µ Ï§??èÜ
  </span>
</div>"""
    _hc.html(_pr_empty_html, height=46)

# ?Ä?Ä P5 ?Ä?Ä
_hc.html(_CSS + _GO_STYLE + "<style>.p5c .go-btn{--go-bg:linear-gradient(270deg,#1565c0,#4fc3f7,#0d47a1,#29b6f6);--go-border:rgba(79,195,247,0.9);}</style>" + _mk_card("p5c","??P5 ?ÑÏû•",
...
# ?Ä?Ä P7 ?Ä?Ä
_hc.html(_CSS + _GO_STYLE + "<style>.p7c .go-btn{--go-bg:linear-gradient(270deg,#6a1b9a,#ce93d8,#4a148c,#ab47bc);--go-border:rgba(155,127,212,0.9);}</style>" + _mk_card("p7c","?ìñ P7 ?ÑÏû•",
    _p7_s1_big,_p7_s1_lbl,_p7_rate_svg,
    _p7_s2_big,_p7_s2_lbl,_p7_cnt_svg,_p7_s3, go="P7_GO") + """
<button class="go-btn" style="background:linear-gradient(270deg,#6a1b9a,#ce93d8,#4a148c,#ab47bc);border-color:rgba(155,127,212,0.9);" onclick="window.parent.document.querySelectorAll('button').forEach(b=>{if((b.innerText||'').trim()==='P7_GO')b.click()})" ontouchend="window.parent.postMessage({action:'goto',page:'p7'},'*');event.preventDefault();" ontouchstart="">?ìñ</button>
<script>
(function(){
  var h=window.innerWidth<=480?70:window.innerWidth<=768?100:140;
  document.querySelector('.card').style.height=h+'px';
...
# ?Ä?Ä ??†Ñ???Ä?Ä
_hc.html(_CSS + _GO_STYLE + "<style>.arc .go-btn{--go-bg:linear-gradient(270deg,#e65100,#ffd54f,#bf360c,#ffca28);--go-border:rgba(255,215,0,0.95);}</style>" + _mk_card("arc","?î• ?§Îãµ?ÑÏû•",
    _arm_s1_big,_arm_s1_lbl,_arm_p5_svg,
    _arm_s2_big,_arm_s2_lbl,_arm_vc_svg,_arm_s3, go="ARM_GO") + """
<button class="go-btn" style="background:linear-gradient(270deg,#e65100,#ffd54f,#bf360c,#ffca28);border-color:rgba(255,215,0,0.95);" onclick="window.parent.document.querySelectorAll('button').forEach(b=>{if((b.innerText||'').trim()==='ARM_GO')b.click()})" ontouchend="window.parent.postMessage({action:'goto',page:'arm'},'*');event.preventDefault();" ontouchstart="">?ó°Ô∏?/button>
<script>
(function(){
  var h=window.innerWidth<=480?70:window.innerWidth<=768?100:140;
  document.querySelector('.card').style.height=h+'px';
