...
def _svg_line(pts: list, color: str, w: int = 200, h: int = 45) -> str:
    """pts: [0~100 or None] Î¶¨Ïä§????SVG ?ºÏù∏Ï∞®Ìä∏ Î¨∏Ïûê??""
    valid = [p for p in pts if p is not None]
    if len(valid) < 2:
        # ?∞Ïù¥???ÜÏúºÎ©??òÌîå ?∞ÏÉÅ???êÏÑ†
...
<div class="wel">
  ...
  <div class="ws">?¥Î≤à??Ï≤??ëÏÜç?¥Ïóê??¬∑ ?ÑÎûò ?ÑÏû• Ï§??òÎÇòÎ•?Í≥®Îùº ?úÏûë?òÏÑ∏???î•</div>
</div>""", height=80)
else:
...
<div class="wel">
  ...
  <div class="ws">?¥Î≤à??Ï¥?{_total}Î¨∏Ï†ú ?ÑÎ£å ¬∑ ?§Îäò???????? ?í™</div>
</div>""", height=80)
...
_P5_ROT = [
    ...
    ("?ÑÏßÅ???êÎ¶¨??", "?ÑÏüÅ?∞Ïóî ?ëÍ≥Ñ ?ÜÎã§!", "??30Ï¥?¬∑ 5Î¨∏Ï†ú ¬∑ ÏßÄÍ∏?Î∞îÎ°ú!"),
...
if _is_first:
    ...
    # ?¨Ï†ë?? ??=?ïÎãµÎ•?Í∑∏Îûò?? ??=?Ä?¥Ïàò+Í∑∏Îûò?? ??=Î°úÌÖå?¥ÏÖò ?êÍ∑πÎ©òÌä∏
...
# ?Ä?Ä P5 ?Ä?Ä
_hc.html(_CSS + _GO_STYLE + _mk_card("p5c","??P5 ?ÑÏû•",
...
# ?Ä?Ä P7 ?Ä?Ä
_hc.html(_CSS + _GO_STYLE + _mk_card("p7c","?ìñ P7 ?ÑÏû•",
    _p7_s1_big,_p7_s1_lbl,_p7_rate_svg if not _is_first else "",
    _p7_s2_big,_p7_s2_lbl,_p7_cnt_svg if not _is_first else "",_p7_s3) + """
<button class="go-btn" onclick="parent.window.parent.document.querySelectorAll('button').forEach(b=>{if((b.innerText||'').trim()==='P7_GO')b.click()})">?ìñ</button>
<script>
(function(){
  var h=window.innerWidth<=480?70:window.innerWidth<=768?100:140;
  document.querySelector('.card').style.height=h+'px';
...
# ?Ä?Ä ??†Ñ???Ä?Ä
_hc.html(_CSS + _GO_STYLE + _mk_card("arc","?ó°Ô∏???†Ñ??,
...
<button class="go-btn" onclick="parent.window.parent.document.querySelectorAll('button').forEach(b=>{if((b.innerText||'').trim()==='ARM_GO')b.click()})">?ó°Ô∏?/button>
<script>
(function(){
  var h=window.innerWidth<=480?70:window.innerWidth<=768?100:140;
  document.querySelector('.card').style.height=h+'px';
...
function hideBtn(){
  window.parent.document.querySelectorAll('button').forEach(b=>{
    ...
    if(t==='ADMIN_GO'||t==='P5_GO'||t==='P7_GO'||t==='ARM_GO'){
      ...
      if(w) w.style.cssText='position:absolute;opacity:0;height:0;overflow:hidden;';
    }
  });
}
