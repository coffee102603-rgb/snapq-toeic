...
import streamlit as st
        _css = f"""<style>#ans-wrap-{_qi}{{display:contents;}}""";
        st.markdown(f'<div id="ans-wrap-{_qi}">', unsafe_allow_html=True)
        _css = f"""<style data-fp>
        /* ── 전장 버튼 래퍼 여백 완전 제거 ── */
        ...
        for _aid, _col, _bg, _sh in _ans_cfg if st.session_state.get("ans", False) else []:
            _css += (
                f'#ans-wrap-{_qi} #btn-{_aid} div[data-testid="stButton"] button{{' 
