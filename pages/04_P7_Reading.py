...
# ═══════════════════════════════════════
    ...
    with bc1:
        if st.session_state[save_key]:
            st.markdown("""<style>div[data-testid="column"]:first-child button[kind="primary"]{animation:savedGlow 2s ease-in-out infinite!important;border:2px solid #00ff66!important;color:#00ff66!important;background:#001a0a!important;}div[data-testid="column"]:first-child button[kind="primary"] p{color:#00ff66!important;}@keyframes savedGlow{0%,100%{box-shadow:0 0 10px rgba(0,255,100,0.4);}50%{box-shadow:0 0 25px rgba(0,255,100,0.9);}}</style>""", unsafe_allow_html=True)
            st.button("✅ 저장완료!", key=f"p7sv_{bi}", type="primary", use_container_width=True, disabled=True)
        else:
            st.markdown("""<style>div[data-testid="column"]:first-child button[kind="primary"]{animation:savePulse 1.2s ease-in-out infinite!important;border:2px solid #ffd700!important;color:#ffd700!important;}div[data-testid="column"]:first-child button[kind="primary"] p{color:#ffd700!important;}@keyframes savePulse{0%,100%{box-shadow:0 0 10px rgba(255,215,0,0.4);}50%{box-shadow:0 0 30px rgba(255,215,0,0.9);}}</style>""", unsafe_allow_html=True)
