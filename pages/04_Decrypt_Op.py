...
# ═══════════════════════════════════════
    ...
    for i, ch in enumerate(cur["choices"]):
        ...
        if st.button(f"【{_btn_labels[i]}】  {_ch_clean}", key=f"p7ch{step}_{i}", use_container_width=True):
            # ═══════════════════════════════════════════════════
            # 🛡️ [BUG FIX 2026.05.10 v5] 다중 클릭 방지 — 강화된 가드
            # 진단 결과: 다중 클릭 시 answers가 누적되어 phase 분기 망가짐
            # 가드 조건:
            #   1. phase가 "battle"이 아니면 (이미 LOST/VICTORY 전환됐으면) 무시
            #   2. 이미 이 step에 답이 있으면 무시
            # 효과: 첫 클릭만 처리, 두 번째 이후 클릭은 자연스럽게 무시
            # ═══════════════════════════════════════════════════
