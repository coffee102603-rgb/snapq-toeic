...
@media(max-width:480px){
  ...
  div[data-testid="stButton"] button.p7t100{min-height:50px!important;}
    ...
    # NPC 광고판
    st.markdown(f"""
<div id='npc-board' style='background:rgba(10,5,20,0.96);border:1.5px solid #0099cc;border-radius:10px;padding:9px 14px;display:flex;align-items:center;gap:10px;margin-bottom:10px;'><span style='font-size:18px;animation:skB 0.9s ease-in-out infinite;display:inline-block;flex-shrink:0;'>💀</span><div style='font-size:12px;font-weight:900;color:#fff;min-height:18px;' id='npc-txt'></div></div><style>@keyframes skB{{0%,100%{{transform:scale(1)}}50%{{transform:scale(1.2)}}}}</style><script>(function(){{var KEY='snapq_tour_day';var today=new Date().toISOString().slice(0,10);var raw=localStorage.getItem(KEY);var data=raw?JSON.parse(raw):{{first:''}};if(!data.first){{data.first=today;localStorage.setItem(KEY,JSON.stringify(data));}}var diff=(new Date(today)-new Date(data.first))/(1000*60*60*24);var msgs=diff<3?["⏱ 100s PRECISION으로 시작! 이해부터!", "📋 유형 파악이 먼저! SIGNAL→BLACKOUT!", "⏱ 시간 욕심 버려! 이해 욕심=점수 급상승!", "📋 유형 훈련=문제 당연 다 맞음!"]:["📡 오늘 해독 임무 시작! 지문 속에 답 있다.", "⏱ 이제 시간 줄여볼 때 됐어! 도전!", "📋 유형별 정복 현황 확인해봐!"];var el=document.getElementById('npc-txt');var mi=0,ci=0;function run(){{var txt=msgs[mi%msgs.length];if(ci<txt.length){{el.textContent+=txt[ci++];setTimeout(run,45);}}else{{setTimeout(function(){{el.textContent='';ci=0;mi++;run();}},3000);}}}}setTimeout(run,600);}})();</script>""", unsafe_allow_html=True)

    # NPC 광고판
    import streamlit.components.v1 as _npc_c
    _npc_c.html("""<div id='npc-board' style='background:rgba(10,5,20,0.96);border:1.5px solid #0099cc;border-radius:10px;padding:9px 14px;display:flex;align-items:center;gap:10px;margin-bottom:4px;'><span style='font-size:18px;animation:skB 0.9s ease-in-out infinite;display:inline-block;flex-shrink:0;'>💀</span><div style='font-size:12px;font-weight:900;color:#fff;min-height:18px;' id='npc-txt'></div></div><style>@keyframes skB{0%,100%{transform:scale(1)}50%{transform:scale(1.2)}}</style><script>(function(){var KEY='snapq_tour_day';var today=new Date().toISOString().slice(0,10);var raw=localStorage.getItem(KEY);var data=raw?JSON.parse(raw):{first:''};if(!data.first){data.first=today;localStorage.setItem(KEY,JSON.stringify(data));}var diff=(new Date(today)-new Date(data.first))/(1000*60*60*24);var msgs=diff<3?["⏱ 100s PRECISION으로 시작! 이해부터!", "📋 유형 파악이 먼저! SIGNAL→BLACKOUT!", "⏱ 시간 욕심 버려! 이해 욕심=점수 급상승!", "📋 유형 훈련=문제 당연 다 맞음!"]:["📡 오늘 해독 임무! 지문 속에 답 있다.", "⏱ 이제 시간 줄여볼 때! 도전!", "📋 유형별 정복 현황 확인해봐!"];var el=document.getElementById('npc-txt');var mi=0,ci=0;function run(){var txt=msgs[mi%msgs.length];if(ci<txt.length){el.textContent+=txt[ci++];setTimeout(run,50);}else{setTimeout(function(){el.textContent='';ci=0;mi++;run();},3000);}}setTimeout(run,600);})();</script>""", height=52)
