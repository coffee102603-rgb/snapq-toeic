...
if st.session_state.sg_phase == "lobby":
    ...
    _nc.html("""<style>#npc-wrap{background:rgba(10,5,20,0.96);border:1.5px solid #8833ff;border-radius:10px;padding:10px 14px;text-align:center;margin-bottom:4px;}#npc-icon{font-size:28px;margin-bottom:4px;animation:iconPop 0.9s ease-in-out infinite;display:block;}@keyframes iconPop{0%,100%{transform:scale(1)}50%{transform:scale(1.18)}}#npc-txt{font-size:13px;font-weight:900;color:#fff;min-height:20px;letter-spacing:0.3px;}</style><div id='npc-wrap'><span id='npc-icon'></span><div id='npc-txt'></div></div><script>(function(){var KEY='snapq_tour_day';var today=new Date().toISOString().slice(0,10);var raw=localStorage.getItem(KEY);var data=raw?JSON.parse(raw):{first:''};if(!data.first){data.first=today;localStorage.setItem(KEY,JSON.stringify(data));}var msgs=[["⚔️🔥", "틀린 문제들 이곳에 다 잡아 뒀어! 박살내!"], ["📡🔍", "암호해독에서 만난 불편한 문장들 똑바로 분석!"], ["🔒💥", "외워질 만큼 반복해! 그놈이 그놈!"], ["📖🏆", "P7문장으로 P5문제 만든다! 둘 다 올려줄게!"]];var ic=document.getElementById('npc-icon');var tx=document.getElementById('npc-txt');var mi=0,ci=0;function run(){var m=msgs[mi%msgs.length];ic.textContent=m[0];if(ci<m[1].length){tx.textContent+=m[1][ci++];setTimeout(run,45);}else{setTimeout(function(){tx.textContent='';ci=0;mi++;run();},3500);}}setTimeout(run,500);})();</script>""", height=80)

    # NPC_BOARD
    _npc_nick = st.session_state.get('student_nickname', '')
    _npc_html = """<div id='npc-board' style='background:rgba(10,5,20,0.96);border:1.5px solid #8833ff;border-radius:10px;padding:10px 14px;text-align:center;margin-bottom:4px;'><span id='npc-icon' style='font-size:28px;margin-bottom:4px;animation:iconPop 0.9s ease-in-out infinite;display:block;'></span><div id='npc-txt' style='font-size:13px;font-weight:900;color:#fff;min-height:20px;'></div></div><style>@keyframes iconPop{0%,100%{transform:scale(1)}50%{transform:scale(1.18)}}</style><script>(function(){var KEY='snapq_tour_day';var today=new Date().toISOString().slice(0,10);var raw=localStorage.getItem(KEY);var data=raw?JSON.parse(raw):{'first':''};if(!data.first){data.first=today;localStorage.setItem(KEY,JSON.stringify(data));}var diff=(new Date(today)-new Date(data.first))/(1000*60*60*24);var nick='__NICK__';var tour=[["⚔️🔥", "틀린 문제들 이곳에 다 잡아 뒀어! 박살내!"], ["📡🔍", "암호해독에서 만난 불편한 문장들 똑바로 분석!"], ["🔒💥", "외워질 만큼 반복해! 그놈이 그놈!"], ["📖🏆", "P7문장으로 P5문제 만든다! 둘 다 올려줄게!"]];var inbody=[["💀⚔️", "__NICK__야, 포로들 심문 준비 됐어?"], ["🔒💥", "__NICK__야, 틀린 문제 반복이 실력!"], ["📖🏆", "__NICK__야, 해독 포로 정복하면 P5도 올라!"]];inbody=inbody.map(function(m){return[m[0],m[1].replace(/__NICK__/g,nick)];});var msgs=diff<3?tour:inbody;var ic=document.getElementById('npc-icon');var tx=document.getElementById('npc-txt');var mi=0,ci=0;function run(){var m=msgs[mi%msgs.length];ic.textContent=m[0];if(ci<m[1].length){tx.textContent+=m[1][ci++];setTimeout(run,45);}else{setTimeout(function(){tx.textContent='';ci=0;mi++;run();},3500);}}setTimeout(run,500);})();</script>"""
