...
st.markdown("""
<style>
.stApp{background:linear-gradient(135deg,#0a0a1a,#1a0a2e 30%,#0a1628 70%,#0a0a1a)!important;color:#e0e0e0!important;}
...
.ah h1{font-size:2rem;font-weight:900;margin:0;background:linear-gradient(90deg,#ff4444,#ff8800,#ffcc00,#44ff44,#4488ff,#aa44ff);background-size:300% 300%;-webkit-background-clip:text;-webkit-text-fill-color:transparent;animation:rb 4s ease infinite;}
@keyframes rb{0%,100%{background-position:0% 50%}50%{background-position:100% 50%}}
.as{color:#666;font-size:0.85rem;}
...
button[kind="primary"] p,button[kind="primary"] span,button[kind="primary"] div{font-size:2.1rem!important;font-weight:900!important;text-align:center!important;}
button[kind="primary"]:hover{box-shadow:0 0 25px rgba(255,68,34,0.7)!important;transform:translateY(-2px)!important;}
button[kind="secondary"]{background:linear-gradient(135deg,#0044cc,#0066ff,#2288ff)!important;color:#fff!important;border:2px solid #3399ff!important;border-radius:14px!important;font-size:2.1rem!important;font-weight:900!important;padding:1.2rem 1.4rem!important;box-shadow:0 0 15px rgba(0,102,255,0.4)!important;text-shadow:0 1px 3px rgba(0,0,0,0.5)!important;text-align:center!important;}
button[kind="secondary"] p,button[kind="secondary"] span,button[kind="secondary"] div{font-size:2.1rem!important;font-weight:900!important;text-align:center!important;}
button[kind="secondary"]:hover{box-shadow:0 0 25px rgba(0,102,255,0.7)!important;transform:translateY(-2px)!important;}
...
.cg{background:linear-gradient(145deg,#1a0000,#2a0a0a);border:1.5px solid #ff4422;box-shadow:0 0 20px rgba(255,68,34,0.15);}
.cv{background:linear-gradient(145deg,#000a1a,#0a0a2a);border:1.5px solid #3399ff;box-shadow:0 0 20px rgba(0,102,255,0.15);}
.ci{font-size:1.8rem;margin-bottom:0.2rem;} .ct{font-size:1.7rem;font-weight:900;margin-bottom:0.4rem;}
.cg .ct{color:#ff6644;} .cv .ct{color:#44aaff;}
.cd{font-size:1.3rem;font-weight:800;color:#ddd;line-height:1.5;}
.ck{font-size:0.9rem;color:#888;margin-top:0.4rem;font-weight:700;}
.cm{background:linear-gradient(145deg,#1a0a1a,#2a1a2a);border:1.5px solid #aa44ff;border-radius:18px;padding:1.2rem;text-align:center;box-shadow:0 0 20px rgba(170,68,255,0.15);animation:fl 3s ease-in-out infinite;}
...
.bt-g{background:linear-gradient(90deg,#1a0000,#2a0808);border:1.5px solid #ff4422;}
.bt-v{background:linear-gradient(90deg,#000a1a,#080828);border:1.5px solid #3399ff;}
.bq{font-size:1.6rem;font-weight:900;} .bq-g{color:#ff6644;} .bq-v{color:#44aaff;}
.bs{font-size:1.1rem;font-weight:800;color:#ffcc00;}
.bk{display:inline-block;background:linear-gradient(135deg,#ff8800,#ffcc00);color:#000;font-weight:900;font-size:0.95rem;padding:0.2rem 0.7rem;border-radius:20px;}

/* 라운드 진행 표시 */
.rd-dots{display:flex;justify-content:center;gap:0.6rem;margin:0.3rem 0;}
.rd-dot{width:22px;height:22px;border-radius:50%;border:2px solid #555;display:flex;align-items:center;justify-content:center;font-size:0.7rem;font-weight:900;}
.rd-cur{border-color:#ffcc00;box-shadow:0 0 8px #ffcc00;}
.rd-ok{background:#22cc44;border-color:#22cc44;color:#fff;}
.rd-no{background:#cc2222;border-color:#cc2222;color:#fff;}
.rd-wait{background:transparent;border-color:#555;color:#555;}

.qb{border-radius:18px;padding:1.8rem 1.5rem;margin:0.2rem 0 0.6rem 0;}
.qb-g{background:linear-gradient(145deg,#1a0505,#2a1010);border:2px solid #ff4422;}
.qb-v{background:linear-gradient(145deg,#050520,#101030);border:2px solid #3399ff;}
.qc{font-size:1.05rem;font-weight:800;margin-bottom:0.8rem;letter-spacing:2px;}
.qc-g{color:#ff6644;} .qc-v{color:#44aaff;}
.qt{color:#f0f0f0;font-size:2.4rem;font-weight:800;line-height:2;word-break:keep-all;}
.qk{color:#ffcc00;font-weight:900;font-size:2.6rem;border-bottom:4px solid #ffcc00;padding-bottom:2px;text-decoration:none;}

/* 브리핑 카드 */
/* 브리핑 배너 */
.br-ban{text-align:center;padding:0.7rem;border-radius:14px;margin:0.3rem 0 0.5rem 0;font-size:1.2rem;font-weight:900;}
.br-ban-v{background:linear-gradient(90deg,rgba(34,204,68,0.2),rgba(255,204,0,0.2));border:1.5px solid #44ff88;color:#44ff88;}
.br-ban-l{background:linear-gradient(90deg,rgba(204,68,68,0.2),rgba(255,136,0,0.2));border:1.5px solid #ff6666;color:#ff6666;}
.br-nav{display:flex;align-items:center;justify-content:center;gap:0.5rem;margin:0.3rem 0;}
.br-dot{width:18px;height:18px;border-radius:50%;display:inline-block;border:2px solid #555;}
.br-dot-ok{background:#22cc44;border-color:#22cc44;}
.br-dot-no{background:#cc4444;border-color:#cc4444;}
.br-dot-cur{box-shadow:0 0 0 3px #ffcc00;transform:scale(1.3);}
.wb{background:#f8f5e8;border-radius:16px;padding:1.8rem 1.5rem;margin:0.5rem 0;box-shadow:0 4px 20px rgba(0,0,0,0.3),inset 0 1px 0 rgba(255,255,255,0.5);border:2px solid #e0d8c0;min-height:250px;}
.wb-qn{font-size:1.4rem;font-weight:800;color:#888;margin-bottom:0.8rem;}
.wb-qn-ok{color:#22aa44;} .wb-qn-no{color:#cc3333;}
.wb-s{font-size:2.15rem;font-weight:700;color:#1a1a1a;line-height:2;margin-bottom:1rem;word-break:keep-all;}
.wb-h{color:#008844;font-weight:900;font-size:2.3rem;text-decoration:underline;text-underline-offset:5px;text-decoration-thickness:3px;text-decoration-color:#00aa55;}
.wb-hn{color:#cc2200;font-weight:900;font-size:2.3rem;text-decoration:underline;text-underline-offset:5px;text-decoration-thickness:3px;text-decoration-color:#cc2200;}
.wb-d{border-top:2px dashed #ccc;margin:0.8rem 0;padding-top:0.8rem;}
.wb-k{font-size:1.6rem;font-weight:600;color:#333;line-height:1.7;margin-bottom:0.6rem;}
.wb-e{font-size:1.5rem;color:#444;line-height:1.6;padding:0.6rem 0.8rem;background:rgba(255,204,0,0.15);border-left:4px solid #ffaa00;border-radius:0 10px 10px 0;}
.brc{background:rgba(255,255,255,0.03);border-radius:14px;padding:1rem;margin:0.5rem 0;}

@media(max-width:768px){
.ah h1{font-size:1.5rem;} .qt{font-size:1.7rem;} .qk{font-size:1.9rem;} .bq{font-size:1.3rem;}
button[kind="primary"],button[kind="secondary"]{font-size:1.5rem!important;padding:0.8rem 1rem!important;}
button[kind="primary"] p,button[kind="primary"] span,button[kind="secondary"] p,button[kind="secondary"] span{font-size:1.5rem!important;}
}
...
if not st.session_state.get("_p5_active", False):
...
if st.session_state.phase=="battle":
    ...
    if not st.session_state.ans:
        ...
        for i,ch in enumerate(q["ch"]):
            with cc[i%2]:
                if st.button(ch,key=f"c{i}_g{st.session_state.get('game_id',0)}",type=bt,use_container_width=True):
                    # 시간초과 재확인
        ...
        # A/B/C/D 선택지 색상 구분 JS
        _bt = "primary" if ig else "secondary"
        components.html(f"""
        <script>
        function colorChoices() {{
            const doc = window.parent.document;
            const kind = '{_bt}';
            const btns = doc.querySelectorAll('button[kind="'+kind+'"]');
            const colors = [
                {{bg:'linear-gradient(135deg,#cc2200,#ee3311)',border:'2px solid #ff5533'}},
                {{bg:'linear-gradient(135deg,#cc6600,#ee7711)',border:'2px solid #ffaa33'}},
                {{bg:'linear-gradient(135deg,#0066aa,#1188cc)',border:'2px solid #33aaee'}},
                {{bg:'linear-gradient(135deg,#6633aa,#8844cc)',border:'2px solid #aa66ee'}}
            ];
            let ci = 0;
            btns.forEach(btn => {{
                const txt = btn.textContent || '';
                if (txt.match(/\\(A\\)|\\(B\\)|\\(C\\)|\\(D\\)/)) {{
                    const c = colors[ci % 4];
                    btn.style.background = c.bg;
                    btn.style.border = c.border;
        ...
        setTimeout(colorChoices, 200);
        setTimeout(colorChoices, 800);
        setTimeout(colorChoices, 2000);
        </script>
        """, height=0)
# ════════════════════════════════════════
...
# ════════════════════════════════════════
    ...
    .stApp button[kind="primary"], .stApp button[kind="secondary"] {
        font-size:1.9rem!important;padding:1rem 1.2rem!important;min-height:auto!important;
    }
    .stApp button[kind="primary"] p, .stApp button[kind="secondary"] p {
...
# ════════════════════════════════════════
    ...
    /* 리모컨 외곽 */
    .rmt{max-width:95vw;margin:0 auto;background:linear-gradient(180deg,#111128,#0e0e22);
        border-radius:32px;padding:24px 16px 16px 16px;border:3px solid #444;
        box-shadow:0 8px 40px rgba(0,0,0,0.5);text-align:center;}
    .rmt-t{font-size:2.6rem;font-weight:900;
        background:linear-gradient(90deg,#ff4466,#ff8844,#ffcc00,#44cc88,#4488ff,#aa44ff);
        background-size:300% 300%;-webkit-background-clip:text;-webkit-text-fill-color:transparent;
        animation:rblb 4s ease infinite;letter-spacing:2px;}
    @keyframes rblb{0%,100%{background-position:0% 50%}50%{background-position:100% 50%}}
    .rmt-s{font-size:0.85rem;color:#667;letter-spacing:1px;}
    .rmt-ir{width:80px;height:4px;background:linear-gradient(90deg,transparent,#ff4488,#ffcc00,#44ffaa,transparent);
        margin:8px auto;border-radius:2px;}

    /* 구역 라벨 - 2배 크게 + 중앙 정렬 */
    .zn{border-radius:18px;padding:18px 14px 14px 14px;margin:16px 0 8px 0;text-align:center;}
    .zn-t{background:linear-gradient(135deg,#152a44,#1a3355);border:2px solid #446688;}
    .zn-b{background:linear-gradient(135deg,#200a0a,#2a1010);border:3px solid #cc4444;}
    .zn-n{background:linear-gradient(135deg,#152a44,#1a3355);border:2px solid #446688;}
    .zl{font-size:1.8rem;font-weight:900;letter-spacing:6px;text-transform:uppercase;text-align:center;}
    .zl-t{color:#99aacc;}
    .zl-b{color:#ff6644;}
    .zl-n{color:#99aacc;}

    /* START 비활성 - 희미한 빨간 */
    .dis-start{text-align:center;background:linear-gradient(135deg,#ff4466,#cc2244);border:3px solid #333;border-radius:50px;
        padding:20px;font-size:2.2rem;opacity:0.35;margin:14px 0;font-weight:900;color:#fff;text-shadow:0 2px 4px rgba(0,0,0,0.4);}

    /* 하단 장식 */
    .rmt-end{background:rgba(0,0,0,0.2);border-radius:0 0 28px 28px;padding:8px;margin-top:10px;}
    .rmt-sp{display:flex;justify-content:center;gap:4px;}
    .rmt-sl{width:55px;height:3px;background:#222;border-radius:2px;}
    ...
    if sm and sm in ["g1","g2","g3","vocab"]:
        ...
        if st.button(lbl_map.get(sm,"START"), key="go_start", type="primary", use_container_width=True):
            md,grp=mode_map[sm]
            st.session_state["game_id"]=st.session_state.get("game_id",0)+1; st.session_state.mode=md; qs=pick5(md,grp)
            st.session_state.round_qs=qs; st.session_state.cq=qs[0]
            st.session_state.qst=time.time(); st.session_state.phase="battle"; st.rerun()
    else:
    ...
    function applyBtnColors() {
        const doc = window.parent.document;
        if (!doc.getElementById('bpSt')) {
            const st = doc.createElement('style'); st.id='bpSt';
            st.textContent = '@keyframes battlePulse{0%,100%{transform:scale(1)}50%{transform:scale(1.02);box-shadow:0 0 25px rgba(255,255,255,0.25)}}';
            doc.head.appendChild(st);
        }
        const btns = doc.querySelectorAll('button[kind="secondary"]');
        const colorMap = {
            '30': {bg:'linear-gradient(135deg,#2a4a6a,#3a5a7a)',shadow:'0 4px 15px rgba(40,70,100,0.3)',sel:'"""+_sel30+"""',extra:'border:2px solid #557799!important;'},
            '40': {bg:'linear-gradient(135deg,#244060,#2e5070)',shadow:'0 4px 15px rgba(35,65,95,0.3)',sel:'"""+_sel40+"""',extra:'border:2px solid #557799!important;'},
            '50': {bg:'linear-gradient(135deg,#1e3858,#284868)',shadow:'0 4px 15px rgba(30,60,90,0.3)',sel:'"""+_sel50+"""',extra:'border:2px solid #557799!important;'},
            '문법력': {bg:'linear-gradient(135deg,#cc0044,#ff1166)',shadow:'0 6px 20px rgba(255,17,102,0.4)',sel:'"""+_selg1+"""',extra:'border:3px solid #fff!important;animation:battlePulse 2s ease-in-out infinite!important;'},
            '구조력': {bg:'linear-gradient(135deg,#cc6600,#ff8811)',shadow:'0 6px 20px rgba(255,136,17,0.4)',sel:'"""+_selg2+"""',extra:'border:3px solid #fff!important;animation:battlePulse 2s ease-in-out infinite!important;'},
            '연결력': {bg:'linear-gradient(135deg,#00884d,#00cc77)',shadow:'0 6px 20px rgba(0,204,119,0.4)',sel:'"""+_selg3+"""',extra:'border:3px solid #fff!important;animation:battlePulse 2s ease-in-out infinite!important;'},
            '어휘력': {bg:'linear-gradient(135deg,#4422aa,#6644dd)',shadow:'0 6px 20px rgba(85,68,204,0.4)',sel:'"""+_selvc+"""',extra:'border:3px solid #fff!important;animation:battlePulse 2s ease-in-out infinite!important;'},
            '저장고': {bg:'linear-gradient(135deg,#2a4a6a,#3a5a7a)',shadow:'0 4px 15px rgba(40,70,100,0.3)',sel:'',extra:'border:2px solid #557799!important;'},
            '메인': {bg:'linear-gradient(135deg,#2a4a6a,#3a5a7a)',shadow:'0 4px 15px rgba(40,70,100,0.3)',sel:'',extra:'border:2px solid #557799!important;'}
        };
        btns.forEach(btn => {
            const txt = btn.textContent || '';
            for (const [key, style] of Object.entries(colorMap)) {
                if (txt.includes(key)) {
                    btn.style.cssText = 'background:'+style.bg+'!important;'+style.extra+'box-shadow:'+style.shadow+'!important;border-radius:20px!important;color:#fff!important;font-size:2rem!important;font-weight:900!important;text-shadow:0 2px 4px rgba(0,0,0,0.4)!important;text-align:center!important;padding:20px 10px!important;min-height:80px!important;'+style.sel;
                    const ps = btn.querySelectorAll('p');
                    ps.forEach(p => { p.style.cssText = 'text-align:center!important;font-size:2rem!important;font-weight:900!important;white-space:pre-line!important;line-height:1.3!important;margin:0!important;'; });
                    break;
                }
            }
        });
        // START 버튼
        const priBtns = doc.querySelectorAll('button[kind="primary"]');
        priBtns.forEach(btn => {
            const txt = btn.textContent || '';
            if (txt.includes('전투') || txt.includes('START')) {
                btn.style.cssText = 'background:linear-gradient(135deg,#cc1133,#ee3355)!important;border:3px solid #8899aa!important;border-radius:50px!important;padding:22px!important;font-size:2.4rem!important;font-weight:900!important;color:#fff!important;text-shadow:0 2px 6px rgba(0,0,0,0.5)!important;letter-spacing:3px!important;box-shadow:0 6px 30px rgba(255,34,68,0.4)!important;text-align:center!important;min-height:75px!important;';
                const ps = btn.querySelectorAll('p');
                ps.forEach(p => { p.style.cssText = 'font-size:2.4rem!important;font-weight:900!important;text-align:center!important;margin:0!important;'; });
            }
        });
    }
    setTimeout(applyBtnColors, 200);
    setTimeout(applyBtnColors, 800);
    setTimeout(applyBtnColors, 2000);
    const obs = new MutationObserver(() => { setTimeout(applyBtnColors, 100); });
