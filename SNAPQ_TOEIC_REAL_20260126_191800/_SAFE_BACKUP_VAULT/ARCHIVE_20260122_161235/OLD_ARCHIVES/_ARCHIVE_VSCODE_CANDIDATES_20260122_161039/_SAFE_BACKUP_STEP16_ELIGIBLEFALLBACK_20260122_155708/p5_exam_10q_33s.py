...
def _start_exam():
    ...
    if len(eligible) < SENTENCES:
        # fail with clear message (safe) — user can expand bank or store more P5 items
        _fail(f"Not enough eligible P5 items with Word Bank targets: {len(eligible)} (need {SENTENCES})")
        return
...
def render():
    ...
    st.markdown(
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin:6px 0 10px 0;">
          <div style="font-weight:950;font-size:34px;color:#f9fafb;">🔥 P5 EXAM · 10Q / 33s</div>
    ...
    st.markdown(
        ...
        <div style="background:rgba(255,255,255,.96);border:3px solid rgba(203,213,225,.92);
          ...
          <div style="font-weight:950;font-size:20px;color:#111827;">Set {si+1}/{SENTENCES} · Q{subq}</div>
