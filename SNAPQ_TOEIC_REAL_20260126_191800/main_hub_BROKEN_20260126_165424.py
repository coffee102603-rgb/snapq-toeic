...
st.markdown("<style>/* HUB_BOTTOM_PAD */.stApp{padding-bottom:220px!important;}.main .block-container{padding-bottom:220px!important;}</style>", unsafe_allow_html=True)
\n﻿# =========================================================
...
def render_lobby():
    ...
    with colR:
        ...
        if st.button("P7\nREADING", use_container_width=True, key="go_p7"):
            safe_record_activity_day(nickname)
            safe_switch("pages/01_P7_Reading_Arena.py")
        st.markdown("</div>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
